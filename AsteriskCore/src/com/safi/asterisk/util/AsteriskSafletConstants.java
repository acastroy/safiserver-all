package com.safi.asterisk.util;

import com.safi.core.saflet.SafletConstants;

public interface AsteriskSafletConstants extends SafletConstants{

  public final static String VAR_KEY_CHANNEL             	= "_channel_";
  public final static String VAR_KEY_REQUEST             	= "_request_";
  public final static String VAR_KEY_MANAGER_CONNECTION  	= "_manager connection_";
  public static final String VAR_KEY_AST_SERVER_ID       	= "_asterisk_server_id_";
	public static final String VAR_KEY_ASTERISK_SERVER 			= "_asterisk_server_";
	public static final String PLATFORM_ID 									= "Asterisk";

}
