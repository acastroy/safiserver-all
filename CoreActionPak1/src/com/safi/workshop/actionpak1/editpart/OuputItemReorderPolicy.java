...
public class OuputItemReorderPolicy extends FlowLayoutEditPolicy {

  public OuputItemReorderPolicy() {
...
//    super.decorateChild(child);
  ...
  protected Command createMoveChildCommand(final EditPart child, final EditPart after) {
    ...
    if (child instanceof OutputItemEditPart && child != after
        && (after == null || after instanceof OutputItemEditPart)) {
      ...
      AbstractTransactionalCommand reorderCommand = new AbstractTransactionalCommand(
        ...
        protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
          ...
          OutputParameter childItem = ((OutputItemEditPart) child).getOutputItemModel();
          
          
          EObject element = getHostObject();
          EList<OutputParameter> list = ((ParameterizedActionstep)element).getOutputParameters();
//          if (element instanceof CustomInitiator){
//            CustomInitiator modelElement = (CustomInitiator) element;
//          
//            list = modelElement.getOutputParameters();
//          }
//          else
//          if (element instanceof IncomingCall2){
//            list = ((IncomingCall2)element).getOutputParameters();
//          }
...
//            compartment.refresh();
            ((InputItemPanelEditPart)getHost()).updateSizeIfNecessary();
            compartment.refresh();
            EditPolicy policy = child.getEditPolicy(PRIMARY_DRAG_ROLE);
            if (policy != null){
//              policy.eraseSourceFeedback(null);
//              policy.eraseTargetFeedback(null);
            }
          }
...
//        || RequestConstants.REQ_DELETE_DEPENDANT.equals(request.getType())
//        || RequestConstants.REQ_ADD.equals(request.getType())
//        || RequestConstants.REQ_ORPHAN_CHILDREN.equals(request.getType())
