...
public class ManagerActionEditorPage extends AbstractActionstepEditorPage {
	...
	private void init(ActionstepEditorDialog parent) {
		...
		final GridData gd_operationLabel = new GridData(SWT.RIGHT, SWT.TOP, false, false);
		...
		ob = ActionstepEditObservables.observeValue(parent.getEditPart().getEditingDomain(),
		    parent.getEditPart().getActionStep(), parent.getEditPart().getActionStep()
		        .eClass().getEStructuralFeature("managerActionType"));
		ISWTObservableValue managerComboElement = SWTObservables.observeSelection(combo);

		managerComboElement = SWTObservables.observeDelayedValue(400, managerComboElement);
		...
		inputItemEditorWidget = new ManagerActionInputParamEditorWidget(this, SWT.NONE);
		...
		// bindingContext.bindList(uiList, modelList, new
		// ManagerActionTargetToModelUpdateStrategy(), null);
		inputItemEditorWidget.setTypeMap(this.getTypeInfo(this.getClassInfo(managerAction
		    .getManagerActionType())));
	...
	private void updateSetters(Class managerActionClass) {
		try {
			...
			final ManagerAction managerAction = (ManagerAction) this.getEditorDialog()
			    .getEditPart().getActionStep();
			...
			if (!list.isEmpty()) {
				TransactionalEditingDomain editingDomain = this.getEditorDialog().getEditPart()
				    .getEditingDomain();
				AddCommand cmd = new AddCommand(editingDomain, managerAction.getInputs(), list);
				editingDomain.getCommandStack().execute(cmd);
			}
			inputItemEditorWidget.setTypeMap(this.getTypeInfo(this.getClassInfo(managerAction
			    .getManagerActionType())));
			// managerAction.getInputs().addAll((Collection<? extends InputItem>)
			// list);
	...
	protected void updateSelectedManagerAction(ManagerActionType selectedManagerAction) {
		// TODO Auto-generated method stub
