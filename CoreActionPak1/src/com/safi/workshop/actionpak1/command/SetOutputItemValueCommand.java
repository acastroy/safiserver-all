...
public class SetOutputItemValueCommand extends
    org.eclipse.gmf.runtime.emf.commands.core.command.AbstractTransactionalCommand {
  private OutputItemEditPart part;
  private DynamicValue newValue;
  private DynamicValue oldValue;
  private boolean canChangeName;

  public SetOutputItemValueCommand(OutputItemEditPart part, boolean canChangeName) {
    super(part.getEditingDomain(), "Open DynamicValueEditor for OutputItem", null);
    this.part = part;
    this.canChangeName = canChangeName;
  }

  public static AbstractOverrideableCommand getEMFCommand(final OutputParameter item,
  ...
  protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
    ...
    final OutputParameter item = part.getOutputItemModel();
  ...
  public static boolean executeCmd(final OutputParameter item, final EditingDomain domain,
          ...
          "Enter parameter name", paramName, new IInputValidator() {
            ...
            public String isValid(String newText) {
              ...
              if (parentToolstep != null) {
                List<OutputParameter> inputs = ((ParameterizedActionstep) parentToolstep)
                    .getOutputParameters();

                for (OutputParameter itm : inputs) {
                  if (itm == item)
                    continue;
                  if (StringUtils.equals(itm.getParameterName(), newText)) {
                    return "OutputParameter with name " + newText
    ...
    List<OutputParameter> inputs = null;

    ActionStep parentToolstep = item.getParentActionStep();

    if (parentToolstep != null && parentToolstep instanceof ParameterizedActionstep) {
      inputs = ((ParameterizedActionstep) parentToolstep).getOutputParameters();
    }

    DynamicValueEditor2 dve = createDynamicValueEditor(inputs == null ? "Output Item"
        : "Output Item #" + inputs.indexOf(item), "Variable Name",
