...
public class EditOutputItemValueCommand extends
    org.eclipse.gmf.runtime.emf.commands.core.command.AbstractTransactionalCommand {
  private OutputParameter item;

  public EditOutputItemValueCommand(OutputParameter item, TransactionalEditingDomain editingDomain) {
    super(editingDomain, "Open DynamicValueEditor for OuptuItem", null);
    this.item = item;
  }
  ...
  public static boolean openEditor(final OutputParameter item,
    ...
    final ParameterizedActionstep parentToolstep = (ParameterizedActionstep) item
        .getParentActionStep();
    final String attributeName = parentToolstep == null ? "Ouput Item" : "Ouput Item #"
        + parentToolstep.getInputs().indexOf(item);
    com.safi.workshop.sheet.DynamicValueEditor2 dve = createDynamicValueEditor(
        attributeName, "Ouput Item", "This item must return a value that can be compared"
    ...
    if (DynamicValueEditor2.OK == result) {
      ...
      if (newval != dynamicValue) {
        AbstractOverrideableCommand cmd = new AbstractOverrideableCommand(editingDomain,
            "Set OutputParameter DynamicValue") {
