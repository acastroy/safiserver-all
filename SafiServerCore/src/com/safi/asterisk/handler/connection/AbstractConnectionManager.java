...
public abstract class AbstractConnectionManager implements AsteriskConnectionManager {
	...
	protected static final long DEBUG_LOOPBACK_TIMEOUT = 120000; // 2 min for
																															 // debugging
	...
	public void handle(String saflet, String astIp, Properties properties,
		...
		if (log.isDebugEnabled())
			log.debug("Invoking custom initiator for saflet " + saflet + " for asterisk "
			    + astIp);
	 ...
	 * 
	 * 
	 * @seecom.safi.asterisk.handler.AsteriskConnectionManager#addListener(com.safi
	 * .server. SafiServerListener)
	 */
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#removeListener(com.
	 * safi.server .SafiServerListener)
	 */
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#getServerIpAddress()
	 */
	...
	protected boolean createManagerConnection(AsteriskServer server) throws Exception {
		synchronized (server) {
			...
			SafiManagerConnectionFactory factory = new SafiManagerConnectionFactory(server
			    .getHostname(), server.getManagerPort(), server.getManagerName(), server
			    .getManagerPassword());
			...
			managerConnection.addListener(new SafiServerListenerInternal(server.getName(),
			    server.getHostname()));
	...
	protected void createAgiServer() throws IOException {
		...
		/*
		 * if (!HandlerEngine.getInstance().isServerMode()) agiServer.startup();
		 * else
		 */{
			agiServerThread = new SafiAgiServerThread(agiServer,
			    new AgiUncaughtExceptionHandler());
	...
	protected void handleLoopback(AgiRequest request, AgiChannel channel) {
		try {
			...
			synchronized (loopbackCallLock) {
				...
				if (obj == null) {
					log.error("No lock found for channel " + channel.getName() + " with uuid "
					    + uuid);
					if (SafletEngine.debuggerLog.isEnabledFor(Level.ERROR))
						SafletEngine.debuggerLog.error("No lock found for channel "
						    + channel.getName() + " with uuid " + uuid);
					return;
				}
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#setLoopbackLock(java
	 * .lang.String)
	 */
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#getLoopbackCall(java
	 * .lang.String)
	 */
	public Object getLoopbackCall(String uuid) {
		...
		synchronized (lock) {
			try {
				if (SafletEngine.debuggerLog.isDebugEnabled())
					SafletEngine.debuggerLog.error("Loopback hasn't arrived for " + uuid
					    + " so i'm waiting!");
		...
		synchronized (safiServerConfig) {
			Runnable runna = new Runnable() {
				...
				public void run() {
					...
					AsteriskServer[] astServers = new AsteriskServer[safiServerConfig
					    .getAsteriskServers().size()];
					...
					for (int i = 0; i < astServers.length; i++) {
						...
						if (managerConnectionMap.containsKey(astServer.getId())) {
							log
							    .warn("Asterisk server with id " + astServer.getId()
							        + " already exists");
						...
						if (StringUtils.isNotBlank(astServer.getHostname())
							...
							try {
								...
								postErrorMessage("License Error",
								    "Couldn't create manager connection for AsteriskServer "
								        + astServer.getName() + " at " + astServer.getHostname());
							}
					}
				}
			};
	...
	protected synchronized void updateSafiServer(SafiServer newConfig)
	    throws DBManagerException {

		boolean isDirty = false;

		if (!StringUtils.equalsIgnoreCase(SafletEngine.getInstance().getBindIP(), newConfig
		    .getBindIP())) {
	...
	public synchronized void safiServerChanged(boolean startup) {
		...
		if (startup
		    || (newConfig != null && safiServerConfig != null && currentFastAgiPort == newConfig
		        .getPort())) {
		...
		if (!startup && newConfig != null && (oldMgrPort != newConfig.getManagementPort())) {
			...
			try {
				...
				e.printStackTrace();
				// log.error("Couldn't init JMX server on new settings...rolling back",
				// e);
		...
		// restart DB
		if (!startup && newConfig != null
		    && DBManager.getInstance().getPort() != newConfig.getDbPort()) {
			try {
	 ...
	 * 
	 * if (log.isDebugEnabled())
	 * log.debug("Someone is rolling back and there are " +
	 ...
	 * log.debug("Rolling back current config " + safiServerConfig + " to " +
	 * oldConfig); } SafiServer newConfig =
	 ...
	 * DBManager.getInstance().saveOrUpdateServerResource(newConfig); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); }
	 * finally { rollbackLock.unlock(); log.debug("Lock is released by " +
	...
	public synchronized void asteriskServerAdded(AsteriskServer astServer) {
		...
		try {
			...
			if (createManagerConnection(astServer))
				this.postInfoMessage("AsteriskServer added and connection created", astServer
				    .getName()
			...
			postErrorMessage("Add Asterisk Server Error",
			    "Couldn't create manager connection for AsteriskServer " + astServer.getName()
			        + " at " + astServer.getHostname() + ": " + e.getLocalizedMessage());
	...
	public synchronized void asteriskServerModified(Integer id) {
		...
		if (connChanged) {
			...
			try {
				...
				e.printStackTrace();
				log.error("Couldn't create manager connection for AsteriskServer " + s.getName()
				    + " at " + s.getHostname(), e);
				postErrorMessage("Modify Asterisk Server Error", "Asterisk server " + s.getName()
				    + " with IP " + s.getHostname() + " could not be connected: "
				    + e.getLocalizedMessage());
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#setDispatcher(com.safi
	 * .asterisk .handler.SafiApplicationDispatch)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @seecom.safi.asterisk.handler.AsteriskConnectionManager#setAgiServer(org.
	 * asteriskjava .fastagi.AgiServer)
	 */
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#setUseBlocking(boolean)
	 */
	 ...
	 * 
	 * @see
	 * com.safi.asterisk.handler.AsteriskConnectionManager#setTesting(boolean)
	 */
					...
					if (ka != null) {
						if (SafletEngine.debuggerLog.isDebugEnabled())
							SafletEngine.debuggerLog
							    .debug("Loopback channel hangup event detected for chan "
	...
	class KeepAlive implements Runnable {
		...
		public void run() {
			...
			if (SafletEngine.debuggerLog.isDebugEnabled())
				SafletEngine.debuggerLog.debug("KeepAlive for channel " + channel.getName()
				    + " stopped...");
		}
	...
	private class ManagerDaemon extends TimerTask {
			...
			synchronized (safiServerConfig) {
				...
				for (int i = 0; i < astServers.length; i++) {
					...
					if (conn == null) {
						try {
							if (createManagerConnection(astServer))
								postInfoMessage("Manager Connection created", astServer.getName()
								    + " with IP " + astServer.getHostname());
						} catch (Exception e) {
							e.printStackTrace();
							log.error("Couldn't create manager connection for server "
							    + astServer.getId(), e);
						}
					} else {
						if (usePing) {
	    ...
	    try {
	    	...
	    	postErrorMessage("Manager Connection Lost",
	    	    "Couldn't create manager connection for Asterisk server "
	    	        + astServer.getName() + " with IP " + astServer.getHostname());
	    }
    }
	}
	...
	class AgiUncaughtExceptionHandler implements java.lang.Thread.UncaughtExceptionHandler {
		...
		public void uncaughtException(Thread t, Throwable e) {
			...
			postErrorMessage("FastAGI Error", "Uncaught FastAGI Error: "
			    + e.getLocalizedMessage());
			// if (oldSafiServerConfig != null && oldSafiServerConfig !=
			// safiServerConfig) {
			// rollbackServerSettings(oldSafiServerConfig);
			// } else {
	...
	class SafiServerListenerInternal implements SafiServerListener {
		...
		public void serverStarted() {
			// postInfoMessage("Asterisk Connection Created", "Connection to
			// AsteriskServer at
		...
		public void serverStopped(String reason) {
			postInfoMessage("Asterisk Connection Lost", "Connection to AsteriskServer named "
			    + name + " at " + ip + " was lost. Reason: " + reason);
		}
	}
