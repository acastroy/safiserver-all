...
public class SafiServerMonitorImpl implements NotificationPublisherAware, SafiServerMonitor, DisposableBean {
  ...
  public void setCurrentThreadCount(int threadCount) {
    ...
    if (publisher != null) {
      javax.management.Notification n = new javax.management.AttributeChangeNotification(this,
          sequenceNumber.getAndIncrement(), System.currentTimeMillis(), "CacheSize changed",
          "CacheSize", "int", oldCount, threadCount);
      publisher.sendNotification(n);
    }
  }

  public void postError(String title, String message) throws UnableToSendNotificationException {
    if (publisher != null) {
      postError(publisher, title, message, this);
    }
  }

  private static void postError(NotificationPublisher publisher, String title, String message,
      Object source) throws UnableToSendNotificationException {
    if (publisher != null) {
      log.error("Error posted: " + message);
      javax.management.Notification n = new javax.management.AttributeChangeNotification(source,
          sequenceNumber.getAndIncrement(), System.currentTimeMillis(), message, title, "Error",
          null, message);
      publisher.sendNotification(n);
    }
  }

  public void postInfo(String title, String message) throws UnableToSendNotificationException {
    if (publisher != null) {
      postInfo(publisher, title, message, this);
    }
  }

  private static void postInfo(NotificationPublisher publisher, String title, String message,
      Object source) throws UnableToSendNotificationException {
    if (publisher != null) {
      log.info("Info posted: " + message);
      javax.management.Notification n = new javax.management.AttributeChangeNotification(source,
          sequenceNumber.getAndIncrement(), System.currentTimeMillis(), message, title, "Info",
          null, message);
      publisher.sendNotification(n);
    }
  }
  ...
  private static void postLogEvent(NotificationPublisher publisher, LoggingEvent event,
    ...
    if (publisher != null) {
      javax.management.Notification n = new javax.management.AttributeChangeNotification(source,
          sequenceNumber.getAndIncrement(), System.currentTimeMillis(), "New LogEvent", "LogEvent",
          LOG_EVENT_TYPE, null, event.getRenderedMessage());
      publisher.sendNotification(n);
    }
  }
  ...
  public static void sendDebugNotification(NotificationPublisher publisher,
    ...
    javax.management.Notification n = new javax.management.AttributeChangeNotification(source,
        sequenceNumber.getAndIncrement(), System.currentTimeMillis(), "New DebugEvent",
        "DebugEvent", DEBUG_EVENT_TYPE, null, wrapper);
    publisher.sendNotification(n);
  }
  ...
  public static void postSystemInfo(NotificationPublisher publisher, SysInfo info, Object source) {
    if (publisher != null) {
      javax.management.Notification n = new javax.management.AttributeChangeNotification(source,
          sequenceNumber.getAndIncrement(), System.currentTimeMillis(), "New SysInfoEvent",
          "SysInfoEvent", SYSINFO_EVENT_TYPE, null, info);
      publisher.sendNotification(n);
    }
  }
  ...
  protected static SysInfo getSystemInfo() {
    ...
    info.setUptime(System.currentTimeMillis() - SafletEngine.getInstance().getStartupTime());
    info.setIpAddress(SafletEngine.getInstance().getBindIP());
    info.setMacAddress(SafletEngine.getMacAddress());
    info.setNumAgiRequests(SafletEngine.getInstance().getConnectionManager().getNumAgiRequests());
  ...
  public void setTracelogLevel(String levelStr) {
    ...
    if (tracelogLevel == Level.OFF) {
      ...
      Enumeration<Logger> enm = Logger.getRootLogger().getLoggerRepository().getCurrentLoggers();
  ...
  public void synchAudioFiles(int asteriskServerID) throws Exception {
    AsteriskServer server = DBManager.getInstance().getAsteriskServerConfig(asteriskServerID);
    if (server == null) {
      throw new DBManagerException("Couldn't find AsteriskServer with ID " + asteriskServerID);
    }
  ...
  public byte[] getPromptFile(int promptID) throws Exception {
    ...
    String filePath = p.getPath() + '.' + (p.getExtension() == null ? "" : p.getExtension());
    ...
    if (!pf.exists()) {
      ...
      for (AsteriskServer server : DBManager.getInstance().getAsteriskServers()) {
        try {
          ...
          log.warn("Couldn't retrieve prompt from Asterisk server " + server.getName(), e);
        }
  ...
  public synchronized int debugSaflet(String projectName, String safletName)
      throws Exception {
    com.safi.db.server.config.Saflet saflet = DBManager.getInstance().getSaflet(projectName,
        safletName);
    if (saflet == null) {
      log.error("Couldn't find Saflet " + projectName + "/" + safletName + " in the DB");
      throw new Exception("Couldn't find Saflet " + projectName + "/" + safletName + " in the DB");
    }

    Saflet handler = SafletEngine.getInstance().getDispatcher().loadHandler(saflet.getCode());
    Object lock = new Object();
    HandlerInfo info = SafletEngine.getInstance().getDispatcher().registerDebugHandler(
        projectName + "/" + safletName, handler, lock, null);
    
    DebugRemoteControl debugControl = new DebugRemoteControlImpl(lock, handler, publisher);
