...
public class FileTransferImpl implements FileTransfer {
  private final static Logger log = Logger.getLogger(FileTransferImpl.class);

  @Override
  public void transferJar(String filename, byte[] data) throws Exception {
    File file = new File(SafletEngine.getInstance().getActionpakDirectory()
  ...
  public void transferWorkspaceArchive(String filename, byte[] data, OverwriteMode mode) throws Exception {
    ...
    File file = new File(SafletEngine.getInstance().getImportDirectory()
        + File.separatorChar +"received"+ File.separatorChar + filename);
  ...
  public boolean needsUpdateActionPak(String filename) {
    File file = new File(SafletEngine.getInstance().getActionpakDirectory()
        + File.separatorChar + filename);
    return !file.exists() && FileUtils.jarIsNewer(filename, 
        SafletEngine.getInstance().getActionpakJars().toArray(
            new File[SafletEngine.getInstance().getActionpakJars().size()]));
  }
  ...
  public void transferAudioFile(String project, String filename, byte[] data) throws Exception {
    String fn = "safi"
        + File.separatorChar
        + (StringUtils.isBlank(project) ? "shared" + File.separatorChar + filename
            : ("project" + File.separatorChar + (project.trim() + File.separatorChar + filename)));
    File file = new File(SafletEngine.getInstance().getAudioDirectoryRoot()
  ...
  @Override
  public void renamePromptFile(int promptId, String projectName, String name, String extension)
      throws Exception {
    Prompt p = DBManager.getInstance().getPromptByID(promptId);
    if (p == null)
      throw new DBManagerException("Couldn't find prompt with id " + promptId);
    String oldName = "safi"
        + File.separatorChar
        + (p.getProject() == null ? "shared" + File.separatorChar + p.getName() + "."
            + p.getExtension() : ("project" + File.separatorChar + (p.getProject().getName()
            + File.separatorChar + p.getName() + "." + p.getExtension())));
    String newName = "safi"
        + File.separatorChar
        + (projectName == null ? "shared" + File.separatorChar + name + "." + extension
            : ("project" + File.separatorChar + (projectName + File.separatorChar + name + "." + extension)));
    String oldNameFull = SafletEngine.getInstance().getAudioDirectoryRoot()
        + File.separatorChar + oldName;
    String newNameFull = SafletEngine.getInstance().getAudioDirectoryRoot()
    ...
    if (!parent.exists() && !parent.mkdirs())
      throw new IOException("Couldn't create directory path: "+parent);
