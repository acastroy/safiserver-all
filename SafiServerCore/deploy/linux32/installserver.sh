...
# Application
APP_NAME="SafiServer"
APP_LONG_NAME="SafiServer"

# Wrapper
WRAPPER_CMD="./deploy/linux32/wrapper"
WRAPPER_CONF="./deploy/linux32/wrapper.conf"
...
if [ "$runfirstsetup" == "true" ];
   ...
   #echo fastagi.port set to: $FastAGIPORT
   echo ""
   ...
   if [ "${#1}" == 0 ]; then
    echo ""
       echo ""
       echo  -e  "  Run ${BLUE}./installserver.sh start${NC} to install and start the SafiServer service"
       exit 0
   fi
   }
fi
