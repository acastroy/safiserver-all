...
public class SafiServerPlugin  {
	...
	public void start() throws Exception {
		...
		// getPreferenceStore().addPropertyChangeListener(new
		// ProdServerPrefListener());
	...
	private int currentSSHDBPortNum = -1;
	private int currentSSHMgmtNum = -1;
	...
	private void refreshDBManagerIfNecessary() {
		...
		DBManager.getInstance().setPort(
		    portNumber < 0 ? store.getDefaultInt(PreferenceConstants.PREF_DB_PORT_PROD)
		        : portNumber, true);
	}

	private void loadGlobalVariables() throws DBManagerException {
		if (hasProductionServerInfo()
		    && StringUtils.isNotBlank(DBManager.getInstance().getHost()))
	...
	public void addGlobalVariable(final Variable v) throws InvocationTargetException,
		...
		// Variable newvar =
		// DBManager.getInstance().changeGlobalVariableName(variable,
		// newName);
...
//				if (sys instanceof SFTPInfo)
	...
	// IDialogSettings dialogSetting =
	// ConsolePlugin.getDefault().getDialogSettings();
	//
	...
	// // }
	// // IDialogSettings
	// dialogSetting=ConsolePlugin.getDefault().getDialogSettings();
	//
...
//			debuggerLog.setLevel(Level.DEBUG);
	...
	// if (debugConsoleAppender == null) {
	// Layout layout = new
	// PatternLayout(AsteriskSafletConstants.DEBUG_PATTERN_LAYOUT);
	// debugConsoleAppender = new WriterAppender(layout,
	// debugConsole.newMessageStream());
	// debuggerLog.addAppender(debugConsoleAppender);
	// }
	// debuggerLog.setLevel(Level.DEBUG);
	// ConsolePlugin.getDefault().getConsoleManager().showConsoleView(debugConsole);
	...
	public void initServerConsole() {
		...
		// // Platform.getExtensionRegistry().removeExtensionPoint(extensionPoint,
		// null);
	...
	public void updateServerResources(IProgressMonitor monitor)
		...
		if (isUseTunnel()) {
			...
			// PreferenceConstants.PREF_SAFISERVER_HOSTNAME_PROD);
			// dbPort =
			// getPreferenceStore().getInt(PreferenceConstants.PREF_DB_PORT_PROD);
		}
		...
		// getPreferenceStore().getString(PreferenceConstants.PREF_SAFISERVER_USER_PROD),
		// true);
		// if (monitor.isCanceled())
		// return;
		// DBManager.getInstance().setPassword(
		// getPreferenceStore().getString(PreferenceConstants.PREF_SAFISERVER_PASS_PROD),
		// true);
		// if (monitor.isCanceled())
		// return;
	...
	private void establishSSHTunnel(IProgressMonitor monitor) throws Exception {
...
//				sshTunnelSession = null;
		...
		currentSSHDBPortNum = updateForwardedPortIfNecessary(currentSSHDBPortNum);
		tunnels.put(sshDBPortRemote, currentSSHDBPortNum);
		// addTunnel(currentSSHDBPortNum, sshDBPortRemote, "127.0.0.1");
		// int sshDBPortNum = -1;
		// if (!isnew) {
		// if (currentSSHDBPortNum > 0) {
		// try {
		...
		// sshTunnelSession.setPortForwardingL(sshDBPortNum, "127.0.0.1",
		// sshDBPortRemote);
		...
		// }
		//
		// sshTunnelSession.setPortForwardingL(sshMgmtPort, "127.0.0.1",
		// sshMgmtPortRemote);
...
//			}
	...
	public synchronized void addTunnel(int localPort, int remotePort, String host)
	    throws Exception {
		...
		if (sshTunnelSession == null || !sshTunnelSession.isConnected())
			...
			// if (bidirectional)
			// sshTunnelSession.setPortForwardingR(localPort, "127.0.0.1",
			// remotePort);
		}
	...
	public boolean isLocalPortForwarded(int localPort) {
		if (localPort < 0 || sshTunnelSession == null || !sshTunnelSession.isConnected()
				...
				public void run() {
					if (d.getActiveShell() != null)
						MessageDialog.openError(d.getActiveShell(), "Management Connection Failure",
						    "Management tunnel to " + getProductionServerDisplayHostname()
						        + " could not be established. Please ensure that port "
						        + currentSSHMgmtNum + " is available on this machine.");
...
//				System.err.println("fwding "+currentSSHMgmtNum+" to remote "+sshMgmtPort);
	...
	// private int updateCurrentSSHMgmtNumIfNecessary() throws IOException {
	// // TODO Auto-generated method stub
	// if (!isLocalPortForwarded(currentSSHMgmtNum) &&
	// !com.safi.server.util.Utils.isServerPortAvail(currentSSHMgmtNum)){
	// currentSSHMgmtNum = Utils.findFreePort();
	// }
	...
	// store.setValue(PreferenceConstants.AST_SFTP_PORT_DEBUG,
	// aserver.getSftpPort());
	...
	// localServer.setName(localServer.getName() == null ? "Debug" :
	// localServer.getName());
	...
	// store.setValue(PreferenceConstants.PREF_FASTAGI_PORT_DEBUG,
	// localServer.getPort());
	...
	public boolean isConnected() {
		return (currentUser != null && currentServer != null && SafiServerRemoteManager
	...
	// String managerName =
	// store.getString(PreferenceConstants.PREF_MGR_USER_DEBUG);
	// String managerPass =
	// store.getString(PreferenceConstants.PREF_MGR_PWD_DEBUG);
	...
	// String sftpPass =
	// store.getString(PreferenceConstants.AST_SFTP_PASSWORD_DEBUG);
	...
	//
	// boolean isPrivate =
	// store.getBoolean(PreferenceConstants.AST_PRIVATE_DEBUG);
	...
	public void updateUser() throws Exception {
		...
		try {
			...
			this.currentUser = (User) criteria.add(
			    Restrictions.and(Restrictions.eq("name", user), Restrictions.eq("password",
			        pass))).uniqueResult();
			if (currentUser != null) {
				if (window != null && window.getShell() != null)
	...
	/*
	 * public void refreshDebugServer() { // if
	 * (!DBManager.getInstance().isInitialized()) { int debugID =
	 * getPreferenceStore().getInt(PreferenceConstants.PREF_AST_ID_DEBUG);
	 * AsteriskServer debugServer = null; boolean connectionSuccessful = false; if
	 * (debugID > 0) { Session session = null; try { session =
	 * DBManager.getInstance().createSession(); connectionSuccessful = true;
	 * debugServer = (AsteriskServer)
	 * session.createCriteria(AsteriskServer.class).add( Restrictions.eq("id",
	 * debugID)).uniqueResult(); if (debugServer == null) {
	 * getPreferenceStore().setValue(PreferenceConstants.PREF_AST_ID_DEBUG, null);
	 * } else { storeDebug(debugServer);
	 * MessageDialog.openInformation(getWorkbench().getDisplay().getActiveShell(),
	 * "Debug Asterisk Server Updated",
	 * "The debug Asterisk server has been updated from the database"); } } catch
	 * (Exception e) { // TODO: handle exception } finally { if (session != null)
	 * { session.close(); session = null; } } }
	 * 
	 * if (debugServer == null && debugID == 0 && connectionSuccessful) {
	 * AsteriskServer newserver = createDebugAsteriskFromStore(); if (newserver !=
	 * null) { Session session = null; SafiServer ss = null; try { session =
	 ...
	 * "SafiServer has no server entry!")); } else { ss = (SafiServer)
	 * results.get(0); ss.getAsteriskServers().add(newserver);
	 * session.saveOrUpdate(ss); session.getTransaction().commit();
	 * getPreferenceStore().setValue(PreferenceConstants.PREF_AST_ID_DEBUG,
	 * newserver.getId()); } } catch (Exception e) { e.printStackTrace();
	 * getLog().log(new Status(Status.ERROR, PLUGIN_ID,
	 * "Couldn't add debug server to DB!", e)); } finally { if (session != null) {
	 * session.close(); session = null; } } } } // } }
	 */
		...
		PlatformUI.getWorkbench().getProgressService().run(true, false,
		    new IRunnableWithProgress() {
			    ...
			    public void run(IProgressMonitor monitor) throws InvocationTargetException,
			        InterruptedException {
				    try {
					    fv[0] = GlobalVariableManager.getInstance().changeGlobalVariableName(
					        variable, newName);
				    } catch (final Exception e) {
					    e.printStackTrace();
					    final Display d = Display.getDefault();
	...
	public int getLocalTunneledManagementPort() {
		if (isUseTunnel()) {
			...
			return currentServer == null ? -1 : currentServer.getManagementPort();
		}
	}
