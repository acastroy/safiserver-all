...
import com.safi.workshop.sqlexplorer.preferences.OverlayPreferenceStore.OverlayKey;
import com.safi.workshop.sqlexplorer.sqleditor.SQLTextViewer;

/**
 * Preferences for SQL Substantial reworking of the preferences page for bug fixing and
 * simplicity; see GeneralPreferencePage for more details.
 * 
 * This page uses OverlayPreferenceStore (copied from the Eclipse source because it's
 * package private) to act as a buffer between the controls onscreen and the
 * IPreferenceStore provided by Eclipse for us to keep the user's choices in. When the
 * user click's OK, the settings in the OverlayPreferenceStore are copied into the
 * Eclipse-provided store.
 * 
 * This allows us to feed the preference store to other code so that we can preview the
 * settings before they are commited.
 * 
 * When adding new fields, add an entry into the PREFERENCES array for the new type of it
...
public class SQLPreferencePage extends OverlaidPreferencePage implements IWorkbenchPreferencePage {
  ...
  // This is a list of all the preferences and their types which available on this page;
  // the OverlayPreferenceStore uses
  // this to determine whether it should store updated values, and if so which method to
  // invoke
  public static final OverlayPreferenceStore.OverlayKey[] PREFERENCES = new OverlayPreferenceStore.OverlayKey[] {
      ...
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_MULTILINE_COMMENT + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_TABLE),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_TABLE + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_COLUMS),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_COLUMS + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_SINGLE_LINE_COMMENT),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_SINGLE_LINE_COMMENT + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_DEFAULT),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_DEFAULT + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_STRING),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_STRING + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.STRING,
          IConstants.SQL_COLOR_KEYWORD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
          IConstants.SQL_COLOR_KEYWORD + BOLD),
      new OverlayPreferenceStore.OverlayKey(OverlayPreferenceStore.BOOLEAN,
  ...
  private final Highlight[] highlights = new Highlight[] {
      ...
      new Highlight(Messages.getString("SQL_Single_Line_Comment_4"),
          IConstants.SQL_COLOR_SINGLE_LINE_COMMENT),
      new Highlight(Messages.getString("SQL_Multi_Line_Comment_5"),
          IConstants.SQL_COLOR_MULTILINE_COMMENT),
  ...
  protected void createFieldEditors() {
    try {
      ...
      syntaxColorList.addSelectionListener(new SelectionAdapter() {
        ...
        public void widgetSelected(SelectionEvent e) {
          ...
          if (i > -1) {
            ...
            syntaxForegroundColorEditor.setColorValue(PreferenceConverter.getColor(
                getPreferenceStore(), highlights[i].id));
          }
        }
      });
      ...
      addField(new BooleanFieldEditor(IConstants.CLIP_EXPORT_COLUMNS, "Export column names",
          exportGroup));
    } catch (Exception e) {
  ...
  private Control createPreviewer(Composite parent) {
    ...
    final String content = Messages.getString("select_*_from_MyTable_--_single_line_comment_12")
        + separator + Messages.getString("/*_multi_line_comment_13") + separator
    ...
    final SQLTextViewer fPreviewViewer = new SQLTextViewer(parent, SWT.V_SCROLL | SWT.H_SCROLL
        | SWT.BORDER, getPreferenceStore(), null);
