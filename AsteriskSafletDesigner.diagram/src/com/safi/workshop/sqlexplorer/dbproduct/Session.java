...
public class Session {
  ...
  private volatile boolean connectionInUse;
  ...
  /**
   * Constructor; ties this Session to a User configuration but does not allocate a SQL
   * connection until required.
   * 
   * @param user
   */
  ...
  /**
   * Returns the current connection, if there is one. Intended only for use by derived
   * classes.
   * 
   * @return
   */
  ...
  /**
   * Grabs a connection; note that releaseConnection MUST be called to return the
   * connection for use by other code.
   * 
   * @return
   * @throws ExplorerException
   */
  // Semaphore semaphore = new Semaphore(1, true);
  ReentrantLock grabLock = new ReentrantLock();

  public SQLConnection grabConnection() throws SQLException {
    if (grabLock.tryLock()) {
      try {
        ...
        if (connection == null && !user.isOfflineMode()) {
          try {
   ...
   * @param toRelease
   */
  public void releaseConnection(SQLConnection toRelease) {
    if (grabLock.tryLock()) {
      try {
        if (!connectionInUse)
  ...
  protected void queueTask(QueuedTask task) throws ExplorerException {
    ...
    if (!connectionInUse && (user == null || !user.isOfflineMode())) {
      try {
        grabConnection();
      } catch (SQLException e) {
        throw new ExplorerException(e);
  ...
  /**
   * Closes the session and returns any connection to the pool. Note that
   * isConnectionInUse() must return false or close() will throw an exception.
   * 
   * @throws ExplorerException
   */
  ...
  /**
   * Forces the connection (if there is one) to be closed, rolling back any open
   * transactions
   * 
   */
  ...
  public synchronized void rollback() throws ExplorerException {
    queueTask(new QueuedTask() {
      ...
      public String getName() {
        // TODO Auto-generated method stub
        return "rollabck connection task";
      }
  ...
  /**
   * Changes the catalog of the connection; this will queue if the connection is currently
   * in use
   * 
   */
  public synchronized void setCatalog(final String catalog) throws ExplorerException {
    ...
    if (catalog != null && connection != null)
      queueTask(new QueuedTask() {
        ...
        public String getName() {
          // TODO Auto-generated method stub
          return "connection set catalog task";
        }
