...
/**
 * Accessor class for obtaining a platform-specific instance of DatabaseProduct. Uses
 * reflection to identify an instance class and then call a public static member called
 * getProductInstance() to get a singleton to return.
 * 
 * If a platform-specific version cannot be found, the DefaultDatabaseProduct is used
 * instead
 * 
 * @see DatabaseProduct
...
public final class DatabaseProductFactory {
  ...
  /**
   * Returns an instance of DatabaseProduct for the platform at the connection held by the
   * connection
   * 
   * @param node
   *          the connected node
  ...
  private static DatabaseProduct getProductInternal(ManagedDriver driver) {
    ...
    // com.safi.workshop.sqlexplorer.database-platform-name.dbproduct.DatabaseProduct
    ...
    try {
      ...
      // Call the method; it's static and has no parameters so both args are null (1st arg
      // is "this")
