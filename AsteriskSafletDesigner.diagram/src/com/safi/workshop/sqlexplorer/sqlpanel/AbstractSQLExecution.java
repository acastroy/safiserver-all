 ...
 * 
 * The AbstractSQLExecution now operates on a QueryTokenizer directly instead of being
 * passed the SQL to execute; this is so that individual queries from a single SQLEditor
 * can run synchronously - this is essential for DDL queries.
 * 
 * This has been decoupled slightly from SQLEditor (which is now refactored to include
 * result tabs) such that an AbstractSQLExecution can simply be told to run by calling
 * startExecute(). The constructor is given the SQLEditor instance that fired it off - and
 * when a results tab is required, SQLEditor.createResultsTab() is called to get one. The
 * old _composite and _parentTab have been replaced by the accessor methods
 * getParentComposite() and getParentTab() respectively which now allocate a tab JIT. The
 * purpose behind this change is that the execution can spark off several tabs and add
 * entries to the Messages tab.
 * 
...
public abstract class AbstractSQLExecution extends Job {
   ...
   * 
   * @param _editor
   *          The SQLEditor that triggered the execution
   * @param statement
  ...
  public IStatus run(IProgressMonitor monitor) {
    ...
    try {
      if (!_session.getUser().isOfflineMode()) {
        ...
        String catalog = _editor.getEditorToolBar().getCurrentCatalog();

        if (catalog != null && !StringUtils.equals(catalog, _connection.getCatalog())) {
      ...
      errorDialog(Messages.getString("SQLResultsView.Error.Title"), e.getClass().getName() + ":"
          + e.getMessage());
  ...
  /**
   * Main execution method. Note that this method is called from a background thread and
   * therefore many SWT operations will need to be done via Display.[a]syncExec()
   * 
   * @param monitor
   * @throws Exception
   */
  ...
  /**
   * This method will be called from the UI thread when execution is cancelled and the tab
   * will be disposed. Do any cleanups required in here. Note that this method is called
   * from a background thread and therefore many SWT operations will need to be done via
   * Display.[a]syncExec()
   * 
   * @throws Exception
   */
  ...
  protected CTabItem allocateResultsTab(Query query) {
    ...
    if (resultsTab == null)
      return null;
    boolean longCaptions = SQLExplorerPlugin.getDefault().getPreferenceStore().getBoolean(
        IConstants.USE_LONG_CAPTIONS_ON_RESULTS);
    if (longCaptions) {
      String caption = resultsTab.getText() + " ["
          + TextUtil.compressWhitespace(query.getQuerySql(), MAX_CAPTION_LENGTH) + "]";
  ...
  /**
   * Handles a SQLException by parsing the message and populating the messages tab; where
   * error messages from the server are numbered, they start relative to the line number
   * of the query that was sent; lineNoOffset is added to each line number so that they
   * relate to the line in SQLEditor
   * 
   * @param e
   */
  protected void logException(SQLException e, String sql) throws SQLException {
    Collection<Message> messages = _session.getDatabaseProduct()
        .getErrorMessages(_connection, e, 0);
    if (messages == null)
      return;
  ...
  protected void logException(ParserException e, Query query) throws SQLException {
    LinkedList<Message> messages = new LinkedList<Message>();
    messages.add(new Message(Message.Status.FAILURE, e.getLineNo(), e.getCharNo(), query
        .getQuerySql(), e.getMessage()));
    addMessages(messages);
  }
   ...
   *          The exception
   * @param query
   *          The Query that triggered the exception
   * @param positionEditor
   *          Whether to reposition the text caret of the editor to the first message
   */
  protected void logException(SQLException e, Query query, boolean positionEditor)
      throws SQLException {
    DatabaseProduct product = _session.getDatabaseProduct();
    if (product == null)
      return;
    final Collection<Message> messages = product.getErrorMessages(_connection, e,
        query.getLineNo() - 1);
    if (messages == null)
      return;
    for (Message message : messages) {
   ...
   * 
   * @param messages
   *          a collection of SQLEditor.Message objects
   */
  ...
  protected void debugLogQuery(Query query, SQLException sqlException) {
    ...
    String level = SQLExplorerPlugin.getDefault().getPreferenceStore().getString(
        IConstants.QUERY_DEBUG_LOG_LEVEL);
