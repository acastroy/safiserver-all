 ...
 * 
 */
public class GenerateSelectQueryAction extends AbstractDBTreeContextAction {
  ...
  private String createTableSelect() {
    ...
    tableName = node.getQualifiedName();
    return query.toString();
  }
  ...
  public String getText() {
    ...
    return Messages.getString("DatabaseStructureView.Actions.GenerateSelectQuery");
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (StringUtils.isNotBlank(tableName)) {
        ...
        action.setQueryName(tableName);
      }

      action.setSql(query);
      action.setUser(_selectedNodes[0].getSession().getUser());
      action.run();
      // SQLEditorInput input = new SQLEditorInput("SQL Editor (" +
      // SQLExplorerPlugin.getDefault().getEditorSerialNo()
      // + ").sql");
