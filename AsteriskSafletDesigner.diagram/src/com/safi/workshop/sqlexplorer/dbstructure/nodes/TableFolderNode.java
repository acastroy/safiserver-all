...
public class TableFolderNode extends AbstractFolderNode {
  ...
  public TableFolderNode(INode parent, String name, MetaDataSession sessionNode, ITableInfo[] tables) {
    ...
    // Nasty temporary hack for Oracle - BIN$ tables are recycle bin tables, and not only
    // does the JDBC driver
    // cause a SQLException when getting meta data on them, it leaks cursors when it does
    // it!
   ...
   * (non-Javadoc)
   * 
   * @see com.safi.workshop.sqlexplorer.dbstructure.nodes.INode#getQualifiedName()
   */
  @Override
  ...
  public void loadChildren() {
    ...
    try {
      ...
      if (_allTables != null && _allTables.length != 0) {
        ...
        tables = _session.getMetaData().getTables(catalogName, schemaName, "%",
            new String[] { _origName }, null);
