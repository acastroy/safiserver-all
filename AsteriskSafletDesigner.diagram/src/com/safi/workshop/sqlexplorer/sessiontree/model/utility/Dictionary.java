...
import com.safi.workshop.sqlexplorer.dbstructure.nodes.SchemaNode;
import com.safi.workshop.sqlexplorer.dbstructure.nodes.TableNode;
import com.safi.workshop.sqlexplorer.sqleditor.SQLCodeScanner;
...
public class Dictionary {
  ...
  static {
    ...
    for (String element : str)
      keywordsTree.put(element, element);
  ...
  public String[] getAllTables() {
    DoublyLinkedList linkedList = tables.getAll();
    int size = linkedList.size();
    DoublyLinkedList.DLLIterator iterator = linkedList.iterator();
    String[] result = new String[size];
    int k = 0;
  ...
  public String[] getAllCatalogSchemas() {
    DoublyLinkedList linkedList = catalogSchemaTree.getAll();
    int size = linkedList.size();
    DoublyLinkedList.DLLIterator iterator = linkedList.iterator();
    String[] result = new String[size];
    int k = 0;
  ...
  public String[] getAllExternalObjects() {
    DoublyLinkedList linkedList = externalObjectTree.getAll();
    int size = linkedList.size();
    DoublyLinkedList.DLLIterator iterator = linkedList.iterator();
    String[] result = new String[size];
    int k = 0;
   ...
   * 
   * @param dbNode
   *          DatabaseNode for which to load the dictionary
  ...
  /**
   * Persists this dictionary so that it can be reused in next sessions without having to
   * be reloaded from database.
   */
  public void store() {
   ...
   * 
   * @param dbNode
   *          DatabaseNode of which to load dictionary information
   * @param monitor
   *          ProgressMonitor displayed whilst loading
  ...
  private void loadSchemaCatalog(INode iNode, IProgressMonitor monitor) throws InterruptedException {
    ...
    if (children != null) {
      ...
      for (INode typeNode : children) {
        ...
        for (String element : SUPPORTED_CONTENT_ASSIST_TYPES) {
          if (typeNode.getType().equalsIgnoreCase(element)) {
