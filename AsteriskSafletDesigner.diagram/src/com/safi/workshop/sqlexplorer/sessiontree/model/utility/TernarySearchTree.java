...
package com.safi.workshop.sqlexplorer.sessiontree.model.utility;
...
/**
 * Implementation of ternary search tree. A Ternary Search Tree is a data structure that
 * behaves in a manner that is very similar to a HashMap.
 */
class TernarySearchTree {

  private TSTNode rootNode;
  private int defaultNumReturnValues = -1;
  private int lastNumberOfReturnValues; // convenience variable for matchPrefix and
  ...
  /**
   * Removes value indexed by key. Also removes all nodes that are rendered unnecessary by
   ...
   *          Set this to -1 to get an unlimited number of return values. Note that the
   *          methods mentioned above provide overloaded versions that allow you to
   *          specify the maximum number of return values, in which case this value is
  ...
  /**
   * Returns the number of values returned by the last call to matchAlmostString or
   * matchPrefixString methods. (This is really just for the purposes of the demo applet.)
   */
  ...
  /**
   * Returns the Node indexed by key, or null if that node doesn't exist. Search begins at
   * root node.
   * 
   * @param key
   *          An index that points to the desired node.
   * @return TSTNode The node object indexed by key. This object is an instance of an
   *         inner class named TernarySearchTree.TSTNode.
   */
  ...
  protected TSTNode getNode(String key, TSTNode startNode) {
    ...
    while (true) {
      if (currentNode == null)
        return null;
      int charComp = CharUtility.compareCharsAlphabetically(key.charAt(charIndex),
  ...
  /**
   * Returns alphabetical list of all keys in the tree that begin with prefix. Only keys
   * for nodes having non-null data are included in the list.
   * 
   * @param prefix
   *          Each key returned from this method will begin with the characters in prefix.
   * @return DoublyLinkedList An implementation of a LinkedList that is java 1.1
   *         compatible.
   */
  ...
  public DoublyLinkedList getAll() {
    DoublyLinkedList sortKeysResult = new DoublyLinkedList();
    TSTNode startNode = rootNode;
    if (startNode == null)
      return sortKeysResult;

    getall(sortKeysResult, startNode);
    return sortKeysResult;
  }

  private void getall(DoublyLinkedList sortKeysResult, TSTNode currentNode) {
    if (currentNode == null)
      return;
    getall(sortKeysResult, currentNode.relatives[TSTNode.LOKID]);
    if (currentNode.data != null)
      sortKeysResult.addLast(getKey(currentNode));

    getall(sortKeysResult, currentNode.relatives[TSTNode.EQKID]);
    getall(sortKeysResult, currentNode.relatives[TSTNode.HIKID]);
  }
   ...
   * @param prefix
   *          Each key returned from this method will begin with the characters in prefix.
   * @return String A string representation of all keys matching the argument prefix. Keys
   *         are delimited with the newline char ('\n').
   */
  ...
  private DoublyLinkedList sortKeysResult; // convenience variable for matchPrefix and
  // sortKeys methods
  private boolean sortKeysList; // convenience variable for matchPrefix, matchPrefixString
  // and sortKeys methods
  private StringBuffer sortKeysBuffer; // convenience variable for matchPrefixString and
  // sortKeys methods
  private int sortKeysNumReturnValues; // convenience variable for matchPrefix and

  // sortKeys methods

  /*
   * Returns keys sorted in alphabetical order. Includes currentNode and all nodes
   * connected to currentNode. Sorted keys will be appended to end of result list. (result
   * may be empty when this method is invoked, but may not be null.)
   */
  ...
  /**
   * Returns keys sorted in alphabetical order. Includes startNode and all nodes connected
   * to startNode. Number of keys returned is limited to numReturnValues. To get a list
   ...
   * Includes startNode and all nodes connected to startNode. Number of keys returned is
   * limited to numReturnValues. To get a list that isn't limited in size, set
   * numReturnValues to -1.
   * 
   * @param startNode
   *          The top node defining the subtree to be searched.
   * @param numReturnValues
  ...
  private boolean matchAlmostListAction; // convenience variable for matchAlmost and
  // matchAlmostString
  ...
  /**
   * Returns a list of keys that almost match argument key. Keys returned will have
   * exactly diff characters that do not match the target key, where diff is equal to the
   * last value passed in as an argument to the setMatchAlmostDiff method. If the
   * matchAlmost method is called before the setMatchAlmostDiff method has been called for
   * the first time, then diff = 0.
   * 
   * @param key
   *          The target key.
   * @return DoublyLinkedList An implementation of a LinkedList that is java 1.1
  ...
  /**
   * Returns a String representation of keys that almost match argument key. Keys returned
   * will have exactly diff characters that do not match the target key, where diff is
   * equal to the last value passed in as an argument to the setMatchAlmostDiff method. If
   * the matchAlmost method is called before the setMatchAlmostDiff method has been called
   * for the first time, then diff = 0.
   * 
   * @param key
   *          The target key.
   * @return String A String representation of keys that almost match the target key. Keys
   *         are delimited by the newline char ('\n').
   */
  ...
  private void matchAlmostRecursion(TSTNode currentNode, int charIndex, int d) {
    ...
    int charComp = CharUtility.compareCharsAlphabetically(matchAlmostKey.charAt(charIndex),
        currentNode.splitchar);
    ...
    if ((matchAlmostKey.length() == charIndex + 1) && (nextD == 0) && (currentNode.data != null)) {
      // Note: the condition nextD == 0 causes keys to be included in the result only if
      // they have exactly matchAlmostDiff number
      // of mismatched letters
      // If instead the condition nextD >= 0 is used, then all keys having up to and
      // including matchAlmostDiff mismatched letters
      // will be included in the result (including a key that is exactly the same as the
      // target string).
  ...
  /**
   * Sets the number of characters by which words can differ from target word when calling
   * matchAlmost or matchAlmostString methods. Arguments less than 1 will set the char
   * difference to 1, and arguments greater than 4 will set the char difference to 4.
   * 
   * @param diff
  ...
  /**
   * Returns the Node indexed by key, creating that node if it doesn't exist, and creating
   * any required. intermediate nodes if they don't exist.
   * 
   * @param key
  ...
  /*
   * Deletes the node passed in as an argument to this method. If this node has non-null
   * data, then both the node and the data will be deleted. Also deletes any other nodes
   * in the tree that are no longer needed after the deletion of the node first passed in
  ...
  private TSTNode deleteNodeRecursion(TSTNode currentNode) {
    // To delete a node, first set its data to null, then pass it into this method, then
    // pass the node returned by this method into this method
    // (make sure you don't delete the data of any of the nodes returned from this
    // method!)
    ...
    // deleteNodeRecursion.
    // (This emulates recursive method call while avoiding the JVM overhead normally
    // associated with a recursive method.)
    if (currentNode == null)
      return null;
    ...
    // if we've made it this far, then we know the currentNode isn't null, but its data
    // and equal kid are null, so we can delete the current node
    ...
    // if we make it this far, we know that EQKID is null, and either HIKID or LOKID is
    // null, or both HIKID and LOKID are NON-null
    if (lokidNull) {
    ...
    int movingKid;
    TSTNode targetNode;
    // if deltaHi is equal to deltaLo, then choose one of them at random, and make it
    // "further away" from the current node's splitchar
    if (deltaHi == deltaLo) {
      if (Math.random() < 0.5) {
        deltaHi++;
      } else {
    ...
    // note that the statements above ensure currentNode is completely dereferenced, and
  ...
  private StringBuffer getKeyBuffer = new StringBuffer(); // convenience variable for
  ...
  /**
   * Returns the total number of nodes in the tree. Counts nodes whether or not they have
   * data.
   * 
   ...
   * Counts nodes whether or not they have data.
   * 
   * @param startingNode
  ...
  /**
   * Returns the number of nodes in the subtree below and including startingNode. Counts
   * only nodes that have non-null data.
   * 
   * @param startingNode
   *          The top node of the subtree. The node that defines the subtree.
   * @return int The total number of nodes in the subtree.
  ...
  /**
   * Prints subtree structure to standard output, beginning with startingNode and workind
   * down.
   */
  ...
  private void printNodeRecursion(TSTNode currentNode) {
    ...
    System.out
        .println("info for node   |" + getKey(currentNode) + "|         node data: " + currentNode.data); //$NON-NLS-1$ //$NON-NLS-2$
    if (currentNode.relatives[TSTNode.PARENT] == null) {
      System.out.println("parent null"); //$NON-NLS-1$
    } else {
      System.out
          .println("parent key   |" + getKey(currentNode.relatives[TSTNode.PARENT]) + "|       parent data: " + currentNode.relatives[TSTNode.PARENT].data); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (currentNode.relatives[TSTNode.LOKID] == null) {
      System.out.println("lokid null"); //$NON-NLS-1$
    } else {
      System.out
          .println("lokid key   |" + getKey(currentNode.relatives[TSTNode.LOKID]) + "|       lo kid data: " + currentNode.relatives[TSTNode.LOKID].data); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (currentNode.relatives[TSTNode.EQKID] == null) {
      System.out.println("eqkid null"); //$NON-NLS-1$
    } else {
      System.out
          .println("eqkid key   |" + getKey(currentNode.relatives[TSTNode.EQKID]) + "|       equal kid data: " + currentNode.relatives[TSTNode.EQKID].data); //$NON-NLS-1$ //$NON-NLS-2$
    }
    if (currentNode.relatives[TSTNode.HIKID] == null) {
      System.out.println("hikid null"); //$NON-NLS-1$
    } else {
      System.out
          .println("hikid key   |" + getKey(currentNode.relatives[TSTNode.HIKID]) + "|       hi kid data: " + currentNode.relatives[TSTNode.HIKID].data); //$NON-NLS-1$ //$NON-NLS-2$
    }
