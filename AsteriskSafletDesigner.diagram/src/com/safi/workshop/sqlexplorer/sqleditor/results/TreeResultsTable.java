...
public class TreeResultsTable extends AbstractResultsTable {
  ...
  protected void createResultsTable(Composite parent) {
    treeViewer = new TreeViewer(parent, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.MULTI
        | SWT.VIRTUAL);
     ...
     * cursor
     * .setForeground(tree.getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION_TEXT));
     * cursor.setLayout(new FillLayout()); cursor.setVisible(false);
     * cursor.addSelectionListener(new SelectionAdapter() { public void
     * widgetSelected(SelectionEvent e) { Table table = treeViewer.getTable();
     * 
     * // when the TableEditor is over a cell, select the corresponding row in // the
     * table table.setSelection(new TableItem[] {cursor.getRow()});
     * cursor.setVisible(true); onSelectCell(cursor); } });
     * 
     * // add resize listener for cursor, to stop cursor from // taking strange shapes
     * after being table is resized cursor.addControlListener(new ControlAdapter() {
     * public void controlResized(ControlEvent e) { if (cursor != null) { if
     * (cursor.getRow() == null) { cursor.setVisible(false); } else { cursor.layout();
     * cursor.redraw(); cursor.setVisible(true); } } } });
     * 
     * // Redisplay the cursor when we get focus tree.addFocusListener(new FocusAdapter()
     * { public void focusGained(FocusEvent e) { Table t = (Table) e.widget; if
     * (t.getItemCount() != 0) { cursor.setVisible(true); } } });
     * cursor.addKeyListener(this); cursor.setMenu(contextMenu);
     */
   ...
   * 
   * @see
   * com.safi.workshop.sqlexplorer.sqleditor.results.ResultsTab#createStatusBar(org.eclipse
  ...
  public void copyToClipboard() {
    ...
    clipBoard.setContents(new Object[] { items[0].getText(columnIndex) },
   ...
   * 
   * @see
   * com.safi.workshop.sqlexplorer.sqleditor.results.ResultsTab#setStatusMessage(java.
   * lang.String)
   */
  @Override
  ...
  /**
   * @param sortableByColumn
   *          the sortableByColumn to set
   */
  ...
  /**
   * @param movableColumns
   *          the movableColumns to set
   */
