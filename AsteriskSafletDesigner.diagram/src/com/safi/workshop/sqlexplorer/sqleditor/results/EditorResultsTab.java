 ...
 * 
 * The ResultsTab class is instantiated when a new Tab for result sets is created; it
 * exists only to return multiple pieces to the calling code.
 * 
 * NOTE: The TabItem is created, its title is the 1-based index of the tab, and its data
 * has been set to the AbstractSQLExecution but it has no other configuration - IE the
 * title is not exactly descriptive, there's no tooltip, etc. Similarly, the parent
 * composite has been created but is completely empty.
 * 
