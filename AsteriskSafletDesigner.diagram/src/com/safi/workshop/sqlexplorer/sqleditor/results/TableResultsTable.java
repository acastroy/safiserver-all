...
public class TableResultsTable extends AbstractResultsTable {
  ...
  protected void createResultsTable(Composite parent) {
    tableViewer = new TableViewer(parent, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.MULTI
        | SWT.VIRTUAL);
  ...
  public CellRange getSelection(SelectionType selection) {
    ...
    switch (selection) {
      ...
      case SELECTION:
        ...
        for (int indecie : indecies) {
          if (rowIndex < 0 || rowIndex > indecie)
            rowIndex = indecie;
          if (bottomRowIndex < indecie)
            bottomRowIndex = indecie;
        }
