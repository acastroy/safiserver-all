...
public class SQLCodeScanner extends AbstractSQLScanner {
  ...
  private static class VersionedWordRule extends WordRule {
    ...
    public VersionedWordRule(IWordDetector detector, String version, boolean enable,
  ...
  static String[] fgTokenProperties = { IConstants.SQL_COLOR_TABLE, IConstants.SQL_COLOR_COLUMS,
      IConstants.SQL_COLOR_KEYWORD, IConstants.SQL_COLOR_STRING, IConstants.SQL_COLOR_DEFAULT,
      IConstants.SQL_PARAMS };

  private VersionedWordRule fVersionedWordRule;
  // private static String[] fgTypes= { "void", "boolean", "char", "byte", "short", "int",
  // "long", "float", "double" };
  ...
  protected synchronized List createRules() {
    ...
    rules.add(new WhitespaceRule(
        new com.safi.workshop.sqlexplorer.sqleditor.SQLWhitespaceDetector()));
    rules.add(new ParamWordRule(new ParamWordDetector(), getToken(IConstants.SQL_PARAMS)));

    // Add word rule for keywords, types, and constants.
    token = getToken(IConstants.SQL_COLOR_DEFAULT);
    // WordRule wordRule= new WordRule(new org.gnu.amaz.SQLEditor.util.SQLWordDetector(),
    // token);
    // UnsignedWordRule wordRule= new UnsignedWordRule(new
    // net.sourceforge.jfacedbc.sqleditor.SQLWordDetector(), token);
    UnsignedWordRule wordRule = new UnsignedWordRule(new SQLWordDetector(), token,
        getToken(IConstants.SQL_COLOR_TABLE), getToken(IConstants.SQL_COLOR_COLUMS), dictionary);

    token = getToken(IConstants.SQL_COLOR_KEYWORD);
    for (String fgKeyword : fgKeywords)
      wordRule.addWord(fgKeyword, token);
    for (String sqlFunction : sqlFunctions)
      wordRule.addWord(sqlFunction, token);
