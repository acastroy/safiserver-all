...
class ExtendedCompletionProposal implements ICompletionProposal {
  ...
  public ExtendedCompletionProposal(String proposalsString, int i, int j, int k, Image tmpImage,
      String str, TableNode tb) {
  ...
  public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int documentOffset) {
    ...
    if (StringUtils.isBlank(string)) {
      ...
      for (int i = 0; i < names.length; i++) {
        tempProps[i] = new CompletionProposal(names[i], documentOffset, 0, names[i].length(),
            catalogImage, names[i], null, null);
      }
      ...
      for (int i = 0; i < names.length; i++) {
        tempProps[i] = new CompletionProposal(names[i], documentOffset, 0, names[i].length(),
            keywordImage, names[i], null, null);
      }
      proposals.addAll(Arrays.asList(tempProps));
    ...
    if (string.charAt(length - 1) == ' ') {
      ...
      if (st != null) {
        ...
        for (int i = 0; i < arr.length; i++) {
          result[i] = new CompletionProposal(arr[i], documentOffset, 0, arr[i].length(), colImage,
              arr[i], null, tableDesc);
        }
        return result;
      }
      ...
      for (String element : proposalsString) {
        ...
        for (int j = 0; j < ls.size(); j++) {
          ...
          ICompletionProposal cmp = new ExtendedCompletionProposal(element,
              documentOffset - length, length, element.length(), tmpImage, element, tbNode);
          propList.add(cmp);
          ...
          for (int j = 0; j < ls.size(); j++) {
            ...
            if (catSchema == node) {
              if (tbNode.isView())
                ...
                tmpImage = tableImage;
              ICompletionProposal cmp = new ExtendedCompletionProposal(element, documentOffset
                  - lastPart.length(), lastPart.length(), element.length(), tmpImage, element,
                  tbNode);
              propList.add(cmp);
            }
          }
