 ...
 * 
 * QueryParsers are not re-usable primarily because the structured comment parser can edit
 * the code and tokens which is generated, therefore removing the possibility of useful
 * caching; they are typically low-cost to allocate a new one and new parsers should be
...
public interface QueryParser extends Iterable<Query> {
   ...
   * 
   * @param originalLineNo
   *          line number in the original code
   * @param numLines
   *          the number of lines added, can be negative
   */
  public void addLineNoOffset(int originalLineNo, int numLines);

  /**
   * Used to adjust a line number returned from the server back to an original source code
   * line number
   * 
   * @param lineNo
   * @return
