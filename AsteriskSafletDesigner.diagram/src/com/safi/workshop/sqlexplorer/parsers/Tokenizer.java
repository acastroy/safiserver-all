...
public class Tokenizer {
  ...
  public static class Token extends BackedCharSequence {
    ...
    public Token(StringBuffer buffer, TokenType tokenType, int start, int end, int lineNo,
        int charNo) {
  ...
  public Token nextToken() throws ParserException {
    ...
    for (; nextToken < sql.length(); nextToken++) {
      ...
      if (Character.isDigit(c)) {
        // If we're not in a token yet, OR: if we'd started punctuation but it's just a
        // dot, then convert to a decimal (missing zero prefix)
        if (tokenType == null
            || (tokenType == TokenType.PUNCTUATION && sql.substring(start, nextToken).equals("."))) {
      ...
      // If we're already on punctuation, stop there (punctuation is always only one
      // character)
    ...
    if (tokenType == null) {
      if (nextToken < sql.length())
        throw new RuntimeException(
            "Internal error: could not find a token but buffer is not exhausted");
      return null;
