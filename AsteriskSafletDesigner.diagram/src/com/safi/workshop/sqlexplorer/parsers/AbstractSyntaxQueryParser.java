...
public abstract class AbstractSyntaxQueryParser extends AbstractQueryParser {
  ...
  public AbstractSyntaxQueryParser(CharSequence sql) {
    this(sql, SQLExplorerPlugin.getDefault().getPreferenceStore().getBoolean(
        IConstants.ENABLE_STRUCTURED_COMMENTS));
  }
  ...
  public void parse() throws ParserException {
    ...
    /*
     * It's important to reset the tokenizer if structured comments are in use because
     * some of the commands in structured comments can cause the SQL to be rewritten; when
     * this happens the start and end locations in any tokens and the starting line
     * numbers in queries and any tokens must be updated to reflect the edits. While we
     * could update any state held by the tokenizer, this is unnecessary if the text has
     * already been fully tokenized - reseting tokenizer to null is just to insist that it
