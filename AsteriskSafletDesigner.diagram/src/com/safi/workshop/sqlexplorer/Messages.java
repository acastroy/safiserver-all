...
public class Messages {
  ...
  /**
   * Get a translated string given its key. This loads the property file for sqlexplorer
   * as well as all known extensions prior to first lookup.
   * 
   * @param key
   *          Message's key.
  ...
  /**
   * Get the translated string given its key and required parameter. This is wrapper
   * around the java {@link MessageFormat} API.
   * 
   * @param key
   *          Message's key.
   * @param param
  ...
  /**
   * Get the translated string given its key and required parameters. This is wrapper
   * around the java {@link MessageFormat} API.
   * 
   * @param key
   *          Message's key.
  ...
  /**
   * Parse some input string and translate all special tokens. This is similar to a string
   * template engine: it extracts <em>key</em> from <tt>${key}</tt> sequences in the input
   * string and uses {@link #getString(String)} to translate it. Contents in between
  ...
  private static void init() {
    if (resources == null) {
      ...
      Bundle mainPlugin = AsteriskDiagramEditorPlugin.getDefault().getBundle();
      ...
      resources[0] = ResourceBundle.getBundle("com.safi.workshop.sqlexplorer.text");
      ...
      for (int i = 0; i < fragments.length; i++) {
        try {
          ...
          // + fragments[i].getSymbolicName() + " [id=" + fragments[i].getBundleId() +
          // "]", e);
        }
      }
    }
