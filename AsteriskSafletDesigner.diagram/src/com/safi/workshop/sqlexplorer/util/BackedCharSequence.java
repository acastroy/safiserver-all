...
/**
 * Implements CharSequence over a section of an existing StringBuffer; note that it does
 * not duplicate the buffer, just provides a view onto a subsection of the StringBuffer.
 * 
 * The idea is that as we tokenize and parse queries, we can use instances of
 * BackedCharSequence to record where the runs of characters for that particular token or
 * query are, without having to duplicate the underlying textual data. This is not solely
 * for memory conservation - because tokens overlap queries, we are able to manipulate the
 * tokens and queries after tokenizing/parsing is complete (EG structured comments).
 * 
...
public class BackedCharSequence implements CharSequence, Iterable<Character> {
  ...
  /**
   * Creates a sequence starting with this and ending with the sequence in end; note that
   * both this and end MUST have the same buffer instance
   * 
   * @param end
   * @return
   */
   ...
   * 
   * @param offset
   *          amount to adjust by, can be negative
   */
