...
public class TableNodeTransfer extends ByteArrayTransfer {
   ...
   * @return true if the native drop data does not match this transfer type. false
   *         otherwise.
   */
   ...
   * Only encode the transfer type name since the selection is read and written in the
   * same process.
   * 
  ...
  /**
   * Overrides org.eclipse.swt.dnd.ByteArrayTransfer#nativeToJava(TransferData). Test if
  ...
  public Object nativeToJava(TransferData transferData) {
    ...
    if (isInvalidNativeType(result)) {
      ...
      return null;
      /*
       * ILog log = ViewsPlugin.getDefault().getLog(); log.log( new Status( IStatus.ERROR,
       * ViewsPlugin.PLUGIN_ID, IStatus.ERROR,
       * ResourceNavigatorMessages.getString("LocalSelectionTransfer.errorMessage"),
       * null)); //$NON-NLS-1$
       */
    }
    return selection;
