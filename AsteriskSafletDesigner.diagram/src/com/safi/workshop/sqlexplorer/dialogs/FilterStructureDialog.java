...
public class FilterStructureDialog extends Dialog {
  ...
  public FilterStructureDialog() {
    ...
    final MetaDataSession session = SQLExplorerPlugin.getDefault().getDatabaseStructureView()
        .getSession();
    if (session != null)
      _db = session.getRoot();
  }
  ...
  public boolean close() {
    ...
    if (items != null) {
      for (TableItem item : items) {
        if (item.getChecked()) {
          schemaSelection.add(item.getText());
        }
      }
    }
    ...
    if (items != null) {
      for (TableItem item : items) {
        if (item.getChecked()) {
          folderSelection.add(item.getText());
        }
      }
    }
  ...
  protected void configureShell(Shell shell) {
    ...
    if (_db == null) {
      shell.setText("No Sessions Available");
      return;
    }
    String title = Messages.getString("FilterStructureDialog.Title.prefix") + " "
        + _db.getSession().toString() + " "
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    try {
      ...
      final TableViewer schemaTable = new TableViewer(composite, SWT.CHECK | SWT.BORDER
          | SWT.V_SCROLL);
      ...
      final TableViewer folderTable = new TableViewer(composite, SWT.CHECK | SWT.BORDER
          | SWT.V_SCROLL);
