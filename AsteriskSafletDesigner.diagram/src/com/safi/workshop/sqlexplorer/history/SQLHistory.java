...
/**
 * SQL History contains all statements that have been executed and is responsible for
...
public class SQLHistory {
  ...
  private int _autoSaveAfterCount = AsteriskDiagramEditorPlugin.getDefault().getPluginPreferences()
  ...
  /**
   * Add a listener to the view, so we can properly refresh it if the sql history has
   * changed.
   * 
   * @param listener
   */
  ...
  /**
   * Add a query string to the sql history. New queries are added to the start of the
   * list, so that the most recent entry is always located on the top of the history list
   * 
   * @param newSql
   *          sql query string
   */
  public void addSQL(String rawSqlString, Session session) {
    ...
    if (rawSqlString == null || rawSqlString.equalsIgnoreCase("commit")
        || rawSqlString.trim().length() == 0) {
      return;
    }

    for (int i = 0; i < _history.size(); i++) {
      SQLHistoryElement el = _history.get(i);
  ...
  public void addSQL(String rawSqlString, Query query) {
    ...
    _history.add(0, new SQLHistoryElement(rawSqlString, query));
  ...
  private void filter() {
    ...
    while (it.hasNext()) {
      ...
      for (String element : keyword) {
        ...
        if ((el.getSearchableString().indexOf(element) == -1)) {
          include = false;
          break;
        }
