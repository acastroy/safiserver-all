...
public class BooleanExpressionEditorPanel extends Composite {
  ...
  public BooleanExpressionEditorPanel(Composite parent, SafletContext context) {
    ...
    treeViewer.addSelectionChangedListener(new ISelectionChangedListener() {
      ...
      public void selectionChanged(final SelectionChangedEvent event) {
        if (isExpressionChanged() && selectedNode != null && selectedNode instanceof ExprNode) {
          if (MessageDialog
              .openQuestion(
                  getShell(),
                  "Apply Changes?",
    ...
    addOperatorButton.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(final SelectionEvent e) {
        IStructuredSelection sel = (IStructuredSelection) parentOperatorComboViewer.getSelection();
        if (sel.isEmpty())
          return;
    ...
    final GridData gd_enableRightVarButton = new GridData(SWT.RIGHT, SWT.CENTER, true, false);
    ...
    rightOperandStackLayout.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 4, 1));
    ...
    executeButton.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(final SelectionEvent e) {
        if (scriptingEnvironment != null && handlerEnvironment != null)
          ScriptEnabledEditorPage.executeRhinoScript(getExpressionText(), scriptingEnvironment,
              handlerEnvironment);
      }
    });
    ...
    final GridData gd_resultantExpressionGroup = new GridData(SWT.FILL, SWT.TOP, true, true, 2, 1);
  ...
  protected boolean isExpressionChanged() {
    if (selectedNode != null && selectedNode instanceof ExprNode) {
      ...
      IStructuredSelection selection = (IStructuredSelection) comboViewer_left.getSelection();
      ...
      if (rightVarMode) {
        ...
        if (en.right instanceof LiteralOperand
            && !StringUtils.equals(((LiteralOperand) en.right).literal, literalText.getText()))
          return true;
  ...
  private StringBuffer buildExpressionHelper(OperatorNode parent, boolean generateExecutable) {
  ...
  private void buildModelFromNode(SimpleNode n, OperatorNode currentParent) {
    if (n instanceof ASTBooleanExpression) {
      ...
      if (sop != null && rightOperand != null) {
        ...
        } else if (rightOperand instanceof ASTNumber || rightOperand instanceof ASTStringLiteral) {
  ...
  protected void updateSetButton() {
    ...
    if (sel.isEmpty()) {
      if ((left instanceof VariableOperand)
          && ((((VariableOperand) left).var == null) || ((((VariableOperand) left).var).getType() != VariableType.BOOLEAN))) {
        setExpressionButton.setEnabled(false);
        return;
      }
    } else
  ...
  private class ExprNode extends Node implements Comparable<Node> {
    ...
    public String getScriptText() {
      return (getLeft() == null ? "" : (getLeft().getScriptText() == null ? "" : getLeft()
          .getScriptText()))
          + " "
          + (operator == null ? "" : operatorToString(operator))
          + " "
          + (getRight() == null ? "" : (getRight().getScriptText() == null ? "" : getRight()
              .getScriptText())).trim();
    }
  ...
  class TreeContentProvider implements IStructuredContentProvider, ITreeContentProvider {
    ...
    public Object[] getChildren(Object parentElement) {
      return ((Node) parentElement).getChildren() == null ? Collections.EMPTY_SET.toArray()
          : ((Node) parentElement).getChildren().toArray();
    }
  ...
  class VariableOperand implements Operand {
    ...
    public String getScriptText() {
      // TODO Auto-generated method stub
      return var == null ? VAR_NOT_FOUND_STR : var.getName();
    }
  ...
  public boolean isExpressionEntered() {
    return (!comboViewer_left.getSelection().isEmpty()
        ...
        || (comboViewer_right.getCombo().isEnabled() && !comboViewer_right.getSelection().isEmpty()) || (literalText
        .isEnabled() && StringUtils.isNotBlank(literalText.getText().trim())))
        && isExpressionChanged();
  }
