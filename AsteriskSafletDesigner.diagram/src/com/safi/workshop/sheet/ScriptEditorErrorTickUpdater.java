 ...
 * to listen on problem changes of the editor's input. It updates the title images when
 * the annotation model changed.
 */
public class ScriptEditorErrorTickUpdater implements IProblemChangedListener {

  private ScriptDynamicValueEditorPage fScriptEditor;
  private ScriptUILabelProvider fLabelProvider;

  public ScriptEditorErrorTickUpdater(ScriptDynamicValueEditorPage editor) {
    Assert.isNotNull(editor);
    fScriptEditor = editor;
    // fLabelProvider= new ScriptUILabelProvider(0,
    // ScriptElementImageProvider.SMALL_ICONS);
    fLabelProvider = DLTKUILanguageManager.createLabelProvider(editor.getLanguageToolkit()
        .getNatureId());
