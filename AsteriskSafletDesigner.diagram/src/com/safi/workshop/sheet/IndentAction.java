...
/**
 * Indents a line or range of lines in a Java document to its correct position. No
 * complete AST must be present, the indentation is computed using heuristics. The
 * algorithm used is fast for single lines, but does not store any information and
 * therefore not so efficient for large line ranges.
 * 
...
public class IndentAction extends TextEditorAction {
  ...
  /**
   * Whether this is the action invoked by TAB. When <code>true</code>, indentation
   * behaves differently to accommodate normal TAB operation.
   */
  ...
  /**
   * Indents a single line using the java heuristic scanner. Javadoc and multiline
   * comments are indented as specified by the <code>JavaDocAutoIndentStrategy</code>.
   * 
   * @param document
   *          the document
   * @param line
   ...
   * @return <code>true</code> if <code>document</code> was modified, <code>false</code>
   *         otherwise
   * @throws BadLocationException
   *           if the document got changed concurrently
   */
  private boolean indentLine(IDocument document, int line, int caret, JavaIndenter indenter,
    ...
    if (offset < document.getLength()) {
      ...
      if (type.equals(IJavaScriptPartitions.JS_DOC)
        ...
        indent = computeJavadocIndent(document, line, scanner, startingPartition);
          ...
          && startingPartition.getType().equals(IJavaScriptPartitions.JS_COMMENT)) {
  ...
  /**
   * Computes and returns the indentation for a javadoc line. The line must be inside a
   * javadoc comment.
   * 
   * @param document
   *          the document
  ...
  /**
   * Returns the size in characters of a string. All characters count one, tabs count the
   * editor's preference for the tab display
   * 
   * @param indent
  ...
  /**
   * Returns a tab equivalent, either as a tab character or as spaces, depending on the
   * editor and formatter preferences.
   * 
  ...
  /**
   * Returns the tab size used by the java editor, which is deduced from the formatter
   * preferences.
   * 
  ...
  private IScriptProject getJavaProject() {
    ...
    ISourceModule cu = DLTKUIPlugin.getDefault().getWorkingCopyManager().getWorkingCopy(
  ...
  /**
   * Returns if the current selection is valid, i.e. whether it is empty and the caret in
   * the whitespace at the start of a line, or covers multiple lines.
   * 
  ...
  /**
   * Returns the document currently displayed in the editor, or <code>null</code> if none
   * can be obtained.
   * 
  ...
  /**
   * Returns the selection on the editor or an invalid selection if none can be obtained.
