...
public class AddBlockCommentAction extends BlockCommentAction {
   ...
   */
  @Override
  protected void runInternal(ITextSelection selection, IDocumentExtension3 docExtension,
    ...
    ITypedRegion partition = docExtension.getPartition(IJavaScriptPartitions.JS_PARTITIONING,
   ...
   * @param offset
   *          the start of the selection, which must lie inside <code>partition</code>
   */
  private void handleFirstPartition(ITypedRegion partition, List edits, Edit.EditFactory factory,
  ...
  /**
   * Handles partition boundaries within the selection. The end of the current partition
   * and the start of the next partition are examined for whether they contain comment
   * tokens that interfere with the created comment.
   * <p>
   * Comment tokens are removed from interior multi-line comments. Javadoc comments are
   * left as is; instead, multi-line comment tokens are inserted before and after Javadoc
   * partitions to ensure that the entire selected area is commented.
   * </p>
   * <p>
   ...
   *           if accessing the document fails - this can only happen if the document gets
   *           modified concurrently
   * @throws BadPartitioningException
   *           if the document does not have a Java partitioning
   */
  private ITypedRegion handleInteriorPartition(ITypedRegion partition, List edits,
      Edit.EditFactory factory, IDocumentExtension3 docExtension) throws BadPartitioningException,
    ...
    partition = docExtension.getPartition(IJavaScriptPartitions.JS_PARTITIONING, partEndOffset,
        false);
  ...
  /**
   * Handles the partition under the end of the selection. For normal java code, the
   * comment end token is inserted at the selection end; if the selection ends inside a
   * special (i.e. string, character, line comment) partition, the entire partition is
   * included inside the comment.
   * 
   * @param partition
   ...
   */
  private void handleLastPartition(ITypedRegion partition, List edits, Edit.EditFactory factory,
    ...
    if (partType == IDocument.DEFAULT_CONTENT_TYPE) {
      ...
      edits.add(factory.createEdit(partition.getOffset() + partition.getLength(), 0,
          getCommentEnd()));
    }
   ...
   * @return <code>true</code> if <code>partType</code> is special, <code>false</code>
   *         otherwise
   */
  ...
  /*
   * @see
   * org.eclipse.jdt.internal.ui.actions.BlockCommentAction#validSelection(org.eclipse
