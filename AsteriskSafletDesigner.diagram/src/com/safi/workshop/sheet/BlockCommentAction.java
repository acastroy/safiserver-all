...
public abstract class BlockCommentAction extends TextEditorAction {
  ...
  /**
   * An edit is a kind of <code>DocumentEvent</code>, in this case an edit instruction,
   * that is affiliated with a <code>Position</code> on a document. The offset of the
   * document event is not stored statically, but taken from the affiliated
   * <code>Position</code>, which gets updated when other edits occur.
   */
  static class Edit extends DocumentEvent {

    /**
     * Factory for edits which manages the creation, installation and destruction of
     * position categories, position updaters etc. on a certain document. Once a factory
     * has been obtained, <code>Edit</code> objects can be obtained from it which will be
     * linked to the document by positions of one position category.
     * <p>
     * Clients are required to call <code>release</code> once the <code>Edit</code>s are
     * not used any more, so the positions can be discarded.
     * </p>
     */
    public static class EditFactory {
      ...
      /**
       * Creates a new <code>EditFactory</code> with an unambiguous position category
       * name.
       * 
       * @param document
       ...
       * 
       * @param offset
       *          the offset of the edition at the point when is created.
       * @param length
       *          the length of the edition (not updated via the position update
       *          mechanism)
       * @param text
      ...
      /**
       * Releases the position category on the document and uninstalls the position
       * updater. <code>Edit</code>s managed by this factory are not updated after this
       * call.
       */
      public void release() {
  ...
  /**
   * Returns the editor's selection, or <code>null</code> if no selection can be obtained
   * or the editor is <code>null</code>.
   * 
  ...
  /**
   * Runs the real command once all the editor, document, and selection checks have
   * succeeded.
   * 
   * @param selection
