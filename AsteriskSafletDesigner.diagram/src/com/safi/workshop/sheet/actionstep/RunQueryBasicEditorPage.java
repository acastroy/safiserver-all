...
public class RunQueryBasicEditorPage extends AbstractActionstepEditorPage {
  ...
  public RunQueryBasicEditorPage(final ActionstepEditorDialog parent) {
    ...
    ob = ActionstepEditObservables.observeValue(editingDomain,
        parent.getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
    ...
    IObservableValue obScrollable = ActionstepEditObservables.observeValue(editingDomain, parent
        .getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
        .getEStructuralFeature("scrollable"));
    ...
    IObservableValue obReadonly = ActionstepEditObservables.observeValue(editingDomain, parent
        .getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
        .getEStructuralFeature("readOnly"));
    ...
    final GridData gd_text_ResultSetName = new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1);
    textResultSetName.setLayoutData(gd_text_ResultSetName);

    IObservableValue obResultSetName = ActionstepEditObservables.observeValue(editingDomain, parent
        .getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
        .getEStructuralFeature("resultSetName"));
    ISWTObservableValue uiResultSetElement = SWTObservables.observeText(textResultSetName,
        SWT.FocusOut);
    ...
    buttonQuery.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(final SelectionEvent e) {
        ...
        EStructuralFeature feature = runQuery.eClass().getEStructuralFeature("connection");
        if (feature != null) {
        ...
        if (result == Window.OK) {
          ...
          // runQueryToolStep.setQuery((DBQueryId) chooser.getSelectedId());

          textQuery.setData(chooser.getSelectedId());
          textQuery.setText(((DBQueryId) chooser.getSelectedId()).getId());
    ...
    IObservableValue obQuery = ActionstepEditObservables.observeValue(parent.getEditPart()
        .getEditingDomain(), parent.getEditPart().getActionStep(), parent.getEditPart()
        .getActionStep().eClass().getEStructuralFeature("query"));
    ISWTObservableValue uiElementQuery = SWTObservables.observeText(textQuery, SWT.Modify);
    ...
    dynamicValueSQL.setEnabled(false);
    dynamicValueSQL.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(runQuery.getSql()));
    ...
    final GridData gd_queryParamsEditorWidget = new GridData(SWT.FILL, SWT.CENTER, false, false, 3,
        1);
    ...
    mappingModelList = ActionstepEditObservables.observeList(editingDomain, runQuery, runQuery
        .eClass().getEStructuralFeature("paramMappings"));
    ...
    mappingUIList = new WritableList(new ArrayList<QueryParamMapping>(runQuery.getParamMappings()),
        QueryParamMapping.class);
    paramBinding = bindingContext.bindList(mappingUIList, mappingModelList, null, null);

    queryParamsEditorWidget.setItemList(mappingUIList);
    queryParamsEditorWidget.setActionstepEditorDialog(parent);

    if (runQuery.getQuery() == null) {
      textQuery.setEnabled(false);
    } else {
      ...
      textQuery.setText(runQuery.getQuery().getId());
