...
public class GetColValuesEditorPage extends AbstractActionstepEditorPage {

  private Combo combo;
  private ComboViewer comboViewer;
  private GetColMappingEditorWidget getColMappingEditorWidget;
  private Label colMappingsLabel;
  private Label resultSetLabel;
  private Text text;
  private Label nameLabel;

  public GetColValuesEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    GetColValues getColValues = (GetColValues) parent.getEditPart().getActionStep();
    TransactionalEditingDomain editingDomain = parent.getEditPart().getEditingDomain();
    IObservableValue ob = ActionstepEditObservables.observeValue(editingDomain, getColValues,
        getColValues.eClass().getEStructuralFeature("name"));
    ...
    getColMappingEditorWidget = new GetColMappingEditorWidget(this, SWT.NONE);
    getColMappingEditorWidget.setEditingDomain(parent.getEditPart().getEditingDomain());
    getColMappingEditorWidget.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
    IObservableList modelList = ActionstepEditObservables.observeList(editingDomain, getColValues,
        getColValues.eClass().getEStructuralFeature("columnMappings"));

    IObservableList uiList = new WritableList(new ArrayList<GetColMapping>(getColValues
        .getColumnMappings()), GetColMapping.class);
    bindingContext.bindList(uiList, modelList, null, null);

    getColMappingEditorWidget.setItemList(uiList);
    getColMappingEditorWidget.setActionstepEditorDialog(parent);

    List<DBResultSetId> resultSets = new ArrayList<DBResultSetId>();
    for (Iterator<EObject> iter = getColValues.eResource().getAllContents(); iter.hasNext();) {
    ...
    ob = ActionstepEditObservables.observeValue(parent.getEditPart().getEditingDomain(),
        getColValues, getColValues.eClass().getEStructuralFeature("resultSet"));
