...
public class FestivalEditorPage extends AbstractActionstepEditorPage {
  ...
  public FestivalEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    // ----------- ActionStep Stuff
    Festival festival = (Festival) parent.getEditPart().getActionStep();
    ...
    digitsLabel.setText("Text to Say:");
    ...
    textDVEWidget.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(festival.getText()));
    textDVEWidget.setEditingDomain(editingDomain);
    textDVEWidget.setObject(festival);

    EStructuralFeature textFeature = festival.eClass().getEStructuralFeature("text");
    textDVEWidget.setFeature(textFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, festival, textFeature);
    ...
    ob = ActionstepEditObservables.observeValue(parent.getEditPart().getEditingDomain(), parent
        .getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
        .getEStructuralFeature("interruptKeys"));
    ISWTObservableValue uiElement3 = SWTObservables.observeText(escapeDigitsText, SWT.FocusOut);
