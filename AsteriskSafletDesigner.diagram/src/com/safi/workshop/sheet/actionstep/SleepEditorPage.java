...
public class SleepEditorPage extends AbstractActionstepEditorPage {

  private Spinner durationText;
  private Combo combo;
  ...
  public SleepEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    TextObservableValue uiElement = (TextObservableValue) SWTObservables.observeText(text,
        SWT.FocusOut);
    ...
    durationmsLabel.setText("Duration (ms):");

    durationText = new Spinner(this, SWT.BORDER);
    durationText.setMaximum(999999999);
    durationText.setIncrement(100);
    final GridData gd_durationText = new GridData(SWT.LEFT, SWT.CENTER, true, false);
    durationText.setLayoutData(gd_durationText);

    IObservableValue ob2 = ActionstepEditObservables.observeValue(parent.getEditPart()
        .getEditingDomain(), parent.getEditPart().getActionStep(), parent.getEditPart()
        .getActionStep().eClass().getEStructuralFeature("duration"));
    ISWTObservableValue uiElement2 = SWTObservables.observeSelection(durationText);
    ...
    // ----------- Native mode Field
    final Label nativeModeLabel = new Label(this, SWT.NONE);
    nativeModeLabel.setText("Native Mode:");

    final Button nativeCheckbox = new Button(this, SWT.CHECK);

    IObservableValue ob3 = ActionstepEditObservables.observeValue(parent.getEditPart()
        .getEditingDomain(), parent.getEditPart().getActionStep(), parent.getEditPart()
        .getActionStep().eClass().getEStructuralFeature("native"));
    ISWTObservableValue uiElement3 = SWTObservables.observeSelection(nativeCheckbox);
    bindingContext.bindValue(uiElement3, ob3, null, null);
