...
public class SayTimeEditorPage extends AbstractActionstepEditorPage {
  ...
  private DynamicValueEditorWidget timeDVEWidget;

  public SayTimeEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    // ----------- ActionStep Stuff
    SayTime saytime = (SayTime) parent.getEditPart().getActionStep();
    ...
    timeLabel.setText("Time to Say:");

    TransactionalEditingDomain editingDomain = parent.getEditPart().getEditingDomain();

    timeDVEWidget = new DynamicValueEditorWidget(this, SWT.NONE);
    timeDVEWidget.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
    timeDVEWidget.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(saytime.getTime()));
    timeDVEWidget.setEditingDomain(editingDomain);
    timeDVEWidget.setObject(saytime);

    EStructuralFeature timeFeature = saytime.eClass().getEStructuralFeature("time");
    timeDVEWidget.setFeature(timeFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, saytime, timeFeature);
    DynamicValueWidgetObservableValue timeVal = new DynamicValueWidgetObservableValue(
        timeDVEWidget, SWT.Modify);

    bindingContext.bindValue(timeVal, ob, null, null);
