...
public class ExtensionTransferEditorPage extends AbstractActionstepEditorPage {
	...
	private DynamicValueEditorWidget channelTypeDVEWidget;
	...
	public ExtensionTransferEditorPage(ActionstepEditorDialog parent) {
		...
		// ----------- ActionStep Stuff
		ExtensionTransfer extensiontransfer = (ExtensionTransfer) parent.getEditPart()
		    .getActionStep();
		...
		ISWTObservableValue uiElement = SWTObservables.observeText(nameText, SWT.FocusOut);
		...
		ob = ActionstepEditObservables.observeValue(parent.getEditPart().getEditingDomain(),
		    parent.getEditPart().getActionStep(), parent.getEditPart().getActionStep()
		        .eClass().getEStructuralFeature("call1"));
		...
		textDVEWidget.setDynamicValue(DynamicValueEditorUtils
		    .copyDynamicValue(extensiontransfer.getExtension()));
		textDVEWidget.setEditingDomain(editingDomain);
		textDVEWidget.setObject(extensiontransfer);

		EStructuralFeature extensionFeature = extensiontransfer.eClass()
		    .getEStructuralFeature("extension");
		textDVEWidget.setFeature(extensionFeature);

		ob = ActionstepEditObservables.observeValue(editingDomain, extensiontransfer,
		    extensionFeature);
		...
		// --------- Additional Options
		{
			...
			optionsDVEWidget.setDynamicValue(DynamicValueEditorUtils
			    .copyDynamicValue(extensiontransfer.getOptions()));
			optionsDVEWidget.setEditingDomain(editingDomain);
			optionsDVEWidget.setObject(extensiontransfer);

			EStructuralFeature optionsFeature = extensiontransfer.eClass()
			    .getEStructuralFeature("options");
			optionsDVEWidget.setFeature(optionsFeature);
		...
		// --------- Additional Options
		{
			final Label channelTypeLabel = new Label(this, SWT.NONE);
			channelTypeLabel.setText("Channel Type:");
			channelTypeLabel.setToolTipText("Asterisk channel type to which call will be transferred (eg. Local (default), Zap, SIP, etc)");

			channelTypeDVEWidget = new DynamicValueEditorWidget(this, SWT.NONE);
			channelTypeDVEWidget.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
			channelTypeDVEWidget.setDynamicValue(DynamicValueEditorUtils
			    .copyDynamicValue(extensiontransfer.getChannelType()));
			channelTypeDVEWidget.setEditingDomain(editingDomain);
			channelTypeDVEWidget.setObject(extensiontransfer);

			EStructuralFeature channelTypeFeature = extensiontransfer.eClass()
			    .getEStructuralFeature("channelType");
			channelTypeDVEWidget.setFeature(channelTypeFeature);

			ob = ActionstepEditObservables.observeValue(editingDomain, extensiontransfer,
			    channelTypeFeature);
			DynamicValueWidgetObservableValue channelTypeVal = new DynamicValueWidgetObservableValue(
					channelTypeDVEWidget, SWT.Modify);

			bindingContext.bindValue(channelTypeVal, ob, null, null);
		}
		...
		preStatusCheckBox
		    .setToolTipText("If checked, the given extension will be queried for it's status before placing the call, using an AMI command");

		IObservableValue preStatusCheckVal = ActionstepEditObservables.observeValue(
		    editingDomain, extensiontransfer, extensiontransfer.eClass()
		        .getEStructuralFeature("doPreExtenStatusCheck"));
		ISWTObservableValue preStatusObservable = SWTObservables
		    .observeSelection(preStatusCheckBox);
