...
public class SayNumberEditorPage extends AbstractActionstepEditorPage {
  ...
  private DynamicValueEditorWidget numberDVEWidget;

  public SayNumberEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    // ----------- ActionStep Stuff
    SayNumber saydigits = (SayNumber) parent.getEditPart().getActionStep();
    ...
    digitsLabel.setText("Number to Say:");

    TransactionalEditingDomain editingDomain = parent.getEditPart().getEditingDomain();

    numberDVEWidget = new DynamicValueEditorWidget(this, SWT.NONE);
    numberDVEWidget.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
    numberDVEWidget
        .setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(saydigits.getNumber()));
    numberDVEWidget.setEditingDomain(editingDomain);
    numberDVEWidget.setObject(saydigits);

    EStructuralFeature textFeature = saydigits.eClass().getEStructuralFeature("number");
    numberDVEWidget.setFeature(textFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, saydigits, textFeature);
    DynamicValueWidgetObservableValue digitsVal = new DynamicValueWidgetObservableValue(
        numberDVEWidget, SWT.Modify);
