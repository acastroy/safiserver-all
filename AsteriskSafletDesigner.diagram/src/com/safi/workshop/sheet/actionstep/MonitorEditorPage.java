...
public class MonitorEditorPage extends AbstractActionstepEditorPage {

  private Text fileFormatText;
  private Combo combo;
  private ComboViewer comboViewer;
  private Label call1Label;
  private Text nameText;
  private Label nameLabel;
  private DynamicValueEditorWidget filePreDVEWidget;

  public MonitorEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    // ----------- ActionStep Stuff
    Monitor monitor = (Monitor) parent.getEditPart().getActionStep();
    ...
    final Label filePreLabel = new Label(this, SWT.NONE);
    filePreLabel.setText("Filename Prefix:");

    filePreDVEWidget = new DynamicValueEditorWidget(this, SWT.NONE);
    filePreDVEWidget.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
    filePreDVEWidget.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(monitor
        .getFilenamePrefix()));
    filePreDVEWidget.setEditingDomain(editingDomain);
    filePreDVEWidget.setObject(monitor);

    EStructuralFeature textFeature = monitor.eClass().getEStructuralFeature("filenamePrefix");
    filePreDVEWidget.setFeature(textFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, monitor, textFeature);
    DynamicValueWidgetObservableValue filepreVal = new DynamicValueWidgetObservableValue(
        filePreDVEWidget, SWT.Modify);

    bindingContext.bindValue(filepreVal, ob, null, null);

    // ----------- Format Field
    final Label fileFormatLabel = new Label(this, SWT.NONE);
    fileFormatLabel.setLayoutData(new GridData());
    fileFormatLabel.setText("File Format:");

    fileFormatText = new Text(this, SWT.BORDER);
    final GridData gd_FileFormatText = new GridData();
    fileFormatText.setLayoutData(gd_FileFormatText);

    ob = ActionstepEditObservables.observeValue(parent.getEditPart().getEditingDomain(), parent
        .getEditPart().getActionStep(), parent.getEditPart().getActionStep().eClass()
        .getEStructuralFeature("format"));
    TextObservableValue uiElement3 = (TextObservableValue) SWTObservables.observeText(
        fileFormatText, SWT.FocusOut);
    ...
    ISWTObservableValue mixmodeUi = SWTObservables.observeSelection(mixModeButton);
    bindingContext.bindValue(mixmodeUi, ob, null, null);
