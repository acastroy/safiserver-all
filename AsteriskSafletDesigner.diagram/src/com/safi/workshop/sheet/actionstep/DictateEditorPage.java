...
public class DictateEditorPage extends AbstractActionstepEditorPage {
  ...
  public DictateEditorPage(ActionstepEditorDialog parent) {
    super(parent);
    ...
    // ----------- ActionStep Stuff
    Dictate dictate = (Dictate) parent.getEditPart().getActionStep();
    ...
    ISWTObservableValue uiElement = (ISWTObservableValue) SWTObservables.observeText(nameText,
        SWT.FocusOut);
    ...
    ISWTObservableValue comboElement = (ISWTObservableValue) SWTObservables
    ...
    dialContextDVEWidget.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(dictate
        .getFilename()));
    dialContextDVEWidget.setEditingDomain(editingDomain);
    dialContextDVEWidget.setObject(dictate);

    EStructuralFeature filenameFeature = dictate.eClass().getEStructuralFeature("filename");
    dialContextDVEWidget.setFeature(filenameFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, dictate, filenameFeature);
    ...
    vmContextDVEWidget.setDynamicValue(DynamicValueEditorUtils.copyDynamicValue(dictate
        .getDirectory()));
    vmContextDVEWidget.setEditingDomain(editingDomain);
    vmContextDVEWidget.setObject(dictate);

    EStructuralFeature directoryFeature = dictate.eClass().getEStructuralFeature("directory");
    vmContextDVEWidget.setFeature(directoryFeature);

    ob = ActionstepEditObservables.observeValue(editingDomain, dictate, directoryFeature);
    DynamicValueWidgetObservableValue directoryVal = new DynamicValueWidgetObservableValue(
        vmContextDVEWidget, SWT.Modify);

    bindingContext.bindValue(directoryVal, ob, null, null);
