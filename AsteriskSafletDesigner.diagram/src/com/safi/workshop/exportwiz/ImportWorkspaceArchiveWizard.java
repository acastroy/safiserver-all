...
public class ImportWorkspaceArchiveWizard extends Wizard {
  ...
  public boolean performFinish() {
    ...
    if (StringUtils.isBlank(filePath)) {
      MessageDialog.openError(getShell(), "No File Selected",
          "Please select a workspace archive file (.sar) file to import");
      return false;
    }
    ...
    if (!file.exists()) {
      MessageDialog
          .openError(
              ...
              "File "
                  + filePath
                  + " could not be found.  Please select a valid workspace archive file (.sar) file to import");
      return false;
    }
    if (importDestinationPage != null && importDestinationPage.isRemote()) {
      try {
        SafiServerRemoteManager.getInstance().transferWorkspaceArchive(file.getName(),
            FileUtils.readFile(filePath), OverwriteMode.FAIL);
      } catch (ImportArchiveException e) {
        e.printStackTrace();
        if (e.getType() == ImportArchiveException.Type.SYSTEM) {
          MessageDialog.openError(getShell(), "Import Failed", "File " + filePath
              + " could not be imported: " + e.getLocalizedMessage());
          return false;
        } else {
          MessageDialog dialog = new MessageDialog(
              ...
              "Name conflict: "
                  ...
                  + ". Do you want to\nSkip: "
                  + "(Resources sharing the same name as existing resources will not be imported)\n"
                  + "or Overwrite (Resources sharing the same name as existing resources will be replaced",
              SWT.ICON_QUESTION, new String[] { "SKIP", "OVERWRITE", "CANCEL" }, 2);
          ...
          try {
            SafiServerRemoteManager.getInstance().transferWorkspaceArchive(file.getName(),
                ...
                result == 0 ? OverwriteMode.SKIP : OverwriteMode.OVERWRITE);
          } catch (Exception e1) {
            e1.printStackTrace();
            MessageDialog.openError(getShell(), "Import Failed", "File " + filePath
                + " could not be imported: " + e.getLocalizedMessage());
      ...
      if (saflets != null && !saflets.isEmpty()) {
        if (MessageDialog
            .openQuestion(
                ...
                "Retrieve Saflets?",
                saflets.size()
                    + " Saflets were imported and uploaded to the SafiServer.\nWould you like to load them into your workspace now?")) {
      ...
      try {
        ...
        e.printStackTrace();
        MessageDialog.openError(getShell(), "DB Import Failed",
            "Database resources could not be imported: " + e.getLocalizedMessage());
        return false;
      }
      return true;
    } else {// do local import
      try {
        ...
        SafletPersistenceManager.getInstance().prepareProjects(getShell(), (List) sps,
        ...
        if (SafiServerPlugin.getDefault().isConnected()) {
          ...
          for (Variable v : vars) {
            ...
            if (var != null) {
              if (!overwriteAll) {
                MessageDialog dialog = new MessageDialog(getShell(), "Global Variable Exists",
                    null, "Variable " + var.getName()
                        + " exists. Do you want to\nSkip, Overwrite, or Overwrite all?",
                    SWT.ICON_QUESTION, new String[] { "SKIP", "OVERWRITE", "OVERWRITE ALL" }, 0);
                int result = dialog.open();
                if (result == 2)
                  overwriteAll = true;
                else if (result == 0)
        ...
        e.printStackTrace();
        MessageDialog.openError(getShell(), "Import Failed", "Resources could not be imported: "
            + e.getLocalizedMessage());
        return false;
      }
  ...
  public SelectArchivePage getSelectArchivePage() {
    if (selectArchivePage == null)
      addPage(selectArchivePage = new SelectArchivePage());
    return selectArchivePage;
  }

  public ViewArchiveContentsPage getViewContentsPage() {
    if (viewContentsPage == null)
      addPage(viewContentsPage = new ViewArchiveContentsPage());
    return viewContentsPage;
  }

  public ImportDestinationPage getImportDestinationPage() {
    if (importDestinationPage == null)
      addPage(importDestinationPage = new ImportDestinationPage());
