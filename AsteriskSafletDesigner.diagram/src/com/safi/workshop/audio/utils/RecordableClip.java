...
public class RecordableClip {
  ...
  public RecordableClip(File file, AudioFormat format) throws IOException, AudioException {
    if (file == null)
      currentFile = File.createTempFile("audioFile", ".wav");
    else
      currentFile = file;
    if (!currentFile.exists()) {
      IOUtils.create(currentFile);
    }
    int len = (int) Math.ceil(format.getFrameSize() * format.getFrameRate()
        * DEFAULT_PROMPT_MINUTES_LEN);
  ...
  public RecordableClip(RecordableClip copy) {
    ...
    // System.arraycopy(copy.streamData, 0, streamData, 0, copy.streamLength);
    this.streamLength = copy.streamLength;
    this.audioFormat = copy.audioFormat;
    if (copy.player != null && copy.player.isOpen())
      copy.player.close();
  }
  ...
  private void stopAudioTracker() {
    if (audioPositionTracker != null)
      audioPositionTracker.stop();
  ...
  public byte[] getSelection(int start, int end) {
    ...
    System
        .arraycopy(streamData.get(), start * audioFormat.getFrameSize(), buffer, 0, buffer.length);
    return buffer;
  }
  ...
  public void cut(int start, int end) {
    ...
    cut.execute();
    undoQueue.add(cut);
    if (undoQueue.size() > MAX_QUEUE_SIZE)
  ...
  public void setAudioData(byte[] data) {
    ...
    // System.arraycopy(data, 0, streamData, 0, data.length);
  ...
  public void deleteSelection(int start, int end) {
    ...
    if (end < len - 1) {
      System.arraycopy(currdata, end * audioFormat.getFrameSize(), newData, idx, (len - end)
  ...
  private void insertSelection(int start, byte[] buffer) {
    ...
    if (start < getLengthInSamples()) {
      ...
      System.arraycopy(currentData, start * audioFormat.getFrameSize(), newData, startIdx
          + buffer.length, getLengthInBytes() - startIdx);
    }
    streamData = new ByteArray(newData);
    streamLength = newData.length;
  ...
  private void muteSelection(int start, int end) {
    for (int i = start * audioFormat.getFrameSize(); i < getLengthInBytes()
        && i < end * audioFormat.getFrameSize(); i++) {
      streamData.get()[i] = 0;
    }
  }
  ...
  public void writeSelection(int start, byte[] data) {
    ...
    // System.arraycopy(data, 0, streamData.get(), start * audioFormat.getFrameSize(),
    // data.length);
  }
  ...
  public void start() throws AudioException {
    if (player == null) {
      try {
        ...
        // File f = File.createTempFile("tempPrompt", ".wav");
        // save(currentFile, AudioFileFormat.Type.WAVE);
        // player = Manager.createRealizedPlayer(f.toURI().toURL());
        // DataLine.Info clipInfo= new DataLine.Info(Clip.class, audioFormat);

        // DataLine.Info srcInfo = new DataLine.Info(SourceDataLine.class, audioFormat);
  ...
  public long getPlayHead() {
    ...
    // return (long)(player.getLongFramePosition() * player.getFormat().getFrameSize() *
    // 0.5d);
    // double pctDone =
    // player.getMicrosecondPosition()/(double)((Clip)player).getMicrosecondLength();

    // return (int) (getLengthInSamples() * pctDone * 2);
  }
  ...
  private static class RecordThread {
    ...
    public void run() {
      try {
        ...
        if (!mSource.getLine().isOpen())
          mSource.getLine().open();
  ...
  class Cut implements UndoableCommand {
    ...
    public void execute() {
      ...
      System.arraycopy(streamData.get(), start * audioFormat.getFrameSize(), oldBuffer, 0,
          oldBuffer.length);
