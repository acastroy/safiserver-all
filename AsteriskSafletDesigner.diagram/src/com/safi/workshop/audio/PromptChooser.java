...
public class PromptChooser extends Composite implements ISelectionChangedListener,
  ...
  public PromptChooser(Composite parent, int style, Mode mode) {
    ...
    editButton.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(final SelectionEvent e) {
        if (selectedNode == null)
          return;
        ImportAudioFileWizard ipt = new ImportAudioFileWizard(ImportAudioFileWizard.Mode.EDIT,
            false);
    ...
    newButton.addSelectionListener(new SelectionAdapter() {
      @Override
      public void widgetSelected(final SelectionEvent e) {
        ImportAudioFileWizard ipt = new ImportAudioFileWizard(ImportAudioFileWizard.Mode.NEW, false);
        WizardDialog dlg = new WizardDialog(AsteriskDiagramEditorUtil.getActiveShell(), ipt);
        int result = dlg.open();
    ...
    synchronizeButton.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(final SelectionEvent e) {
        ...
        try {
          ...
          // TODO Auto-generated catch block
          e1.printStackTrace();
          AsteriskDiagramEditorPlugin.getInstance().logError("Couldn't retrieve all prompts", e1);
    ...
    if (mode == Mode.MULTIPLE) {
      ...
      ((CheckboxTreeViewer) treeViewer).addCheckStateListener(new ICheckStateListener() {
        ...
        public void checkStateChanged(CheckStateChangedEvent event) {
          ...
          Object[] checkedElements = ((CheckboxTreeViewer) treeViewer).getCheckedElements();
    ...
    // GridData layoutData = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
    // layoutData.widthHint=200;
    // promptController.setLayoutData(layoutData);
    //
    // if (projectCache != null)
    categories = new HashMap<PromptNode, String>();
    // autocomplete = new AutoCompleteField(filterText, new TextContentAdapter(),
  ...
  public void dispose() {
    if (needsSynch && SafiServerPlugin.getDefault().hasAsteriskServers()) {
      boolean b = MessageDialog.openQuestion(AsteriskDiagramEditorUtil.getActiveShell(),
          "Synchronize Now?", "One or more prompts were changed and may need to be synchronized "
    ...
    IRunnableWithProgress runna = new IRunnableWithProgress() {
      ...
      public void run(IProgressMonitor monitor) throws InvocationTargetException,
        ...
        try {
          ...
          PromptCache.projectPromptCache.put(((SafletProject) promptNode.serverResource).getId(),
              cache);
          result[0] = cache;
  ...
  private String getPath(PromptNode promptNode) {
    ...
    // buf = new StringBuffer(String.valueOf(promptNode.serverResource.getId()));
    // else
    ...
    if (promptNode.mode == NodeMode.PROMPT && !((Prompt) promptNode.serverResource).isSystem()) {
      buf.insert(0, "safi/");
    }
    return buf.toString();
  }
  ...
  private class PromptNode {
      ...
      if (children == null) {
        try {
          ...
          MessageDialog.openError(getShell(), "Retrieve Error",
              "Couldn't retrieve all children of " + name + ": " + e.getLocalizedMessage());
        }
      }
    }
    ...
    public String toString() {
      return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).append("name", name)
          .append("mode", mode).append("project", project).toString();
    }
    ...
    d.syncExec(new Runnable() {
      ...
      public void run() {
        ...
        if (initialNode != null) {
          ...
          treeViewer.setSelection(new StructuredSelection(initialNode), true);
    ...
    public boolean select(Viewer viewer, Object parentElement, Object element) {
      ...
      try {
        ...
        if (node.serverResource != null && node.mode == NodeMode.PROMPT && !node.project
            && !((Prompt) node.serverResource).isSystem() && sharedCheckbox.getSelection()) {
          // hide shared
          return false;
        }
  ...
  private PromptNode findNodeHelper(Prompt payload, PromptNode current) {
    if (current.mode == NodeMode.PROMPT && payload.getId() == current.serverResource.getId())
      return current;
    ...
    public int compare(Viewer viewer, Object e1, Object e2) {
      ...
      if (this.column == ColumnID.NAME) {
        if (n1.mode == NodeMode.PROMPT && n2.mode != NodeMode.PROMPT)
          ...
          returnValue = n1.serverResource.getName().compareTo(n2.serverResource.getName());
        ...
        if (n1.mode == NodeMode.PROMPT && n2.mode != NodeMode.PROMPT)
          ...
          returnValue = (user1 == null ? "" : user1.getName()).compareTo((user2 == null ? ""
              : user2.getName()));
        }
      }
