 ...
 * @generated
 */
public class AsteriskDocumentProvider extends AbstractDocumentProvider implements
  ...
  protected ElementInfo createElementInfo(Object element) throws CoreException {
    if (false == element instanceof URIEditorInput) {
      throw new CoreException(new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID, 0, NLS
          .bind(Messages.AsteriskDocumentProvider_IncorrectInputError, new Object[] { element,
  ...
  /**
   * Sets up the given document as it would be provided for the given element. The content
   * of the document is not changed. This default implementation is empty. Subclasses may
   * reimplement.
   * 
   * @param element
  ...
  private TransactionalEditingDomain createEditingDomain() {
    TransactionalEditingDomain editingDomain = AsteriskDiagramEditingDomainFactory.getInstance()
        .createEditingDomain();
    editingDomain.setID("AsteriskSafletDesigner.diagram.EditingDomain"); //$NON-NLS-1$
      ...
      try {
        ...
        if (!resource.isLoaded()) {
          try {
            ...
            // Boolean.TRUE);
            resource.setURI(uri.trimFragment());
        ...
        throw new RuntimeException(Messages.AsteriskDocumentProvider_NoDiagramInResourceError);
      } catch (Exception e) {
        ...
        if (e instanceof CoreException) {
          ...
          thrownExcp = new CoreException(new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID,
              0, msg != null ? msg : Messages.AsteriskDocumentProvider_DiagramLoadingError, e));
        }
        throw thrownExcp;
      }
    } else {
      ...
      if (info.isUpdateCache()) {
        try {
          ...
          AsteriskDiagramEditorPlugin.getInstance().logError(
              Messages.AsteriskDocumentProvider_isModifiable, ex);
    ...
    if (info != null) {
      if (!overwrite && !info.isSynchronized()) {
        throw new CoreException(new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID,
            IStatus.ERROR, Messages.AsteriskDocumentProvider_UnsynchronizedFileSaveError, null));
      }
      ...
      if (AsteriskDiagramEditorUtil.hasDebugFile(info.getResourceSet())) {
        Display.getDefault().asyncExec(new Runnable() {
          @Override
          public void run() {
            MessageDialog.openWarning(Display.getDefault().getActiveShell(), "Debug Window",
                "Debug handlers cannot be saved.  Any modifications to this file will be lost");
      ...
      try {
        monitor.beginTask(Messages.AsteriskDocumentProvider_SaveDiagramTask, resources.size() + 1); // "Saving
        // diagram"
        for (Iterator it = resources.iterator(); it.hasNext();) {
          Resource nextResource = (Resource) it.next();
          monitor.setTaskName(NLS.bind(Messages.AsteriskDocumentProvider_SaveNextResourceTask,
              nextResource.getURI()));
      ...
      try {
        new AbstractTransactionalCommand(diagramDocument.getEditingDomain(), NLS.bind(
            Messages.AsteriskDocumentProvider_SaveAsOperation, diagramCopy.getName()),
            affectedFiles) {
          @Override
      ...
      if (!notification.isTouch() && myModifiedFilter.matches(notification)) {
        if (notification.getNotifier() instanceof Resource) {
          ...
          if (resource.isLoaded()) {
            ...
            for (Iterator it = myInfo.getResourceSet().getResources().iterator(); it.hasNext()
                && !modified;) {
              Resource nextResource = (Resource) it.next();
