 ...
 * The Thumbnail will maintain the aspect ratio of the source Figure.
 * 
 * @author Eric Bordeau
 */
public class SafiThumbnail extends Figure implements UpdateListener {

  /**
   * This updates the SafiThumbnail by breaking the thumbnail {@link Image} into several
   * tiles and updating each tile individually.
   */
  class ThumbnailUpdater implements Runnable {
    ...
    /**
     * Returns <code>true</code> if this ThumbnailUpdater is active. An inactive updater
     * has disposed of its {@link Image}. The updater may be active and not currently
     * running.
     * 
    ...
    /**
     * Returns <code>true</code> if this is currently running and updating at least one
     * tile on the thumbnail {@link Image}.
     * 
    ...
    /**
     * Resets the number of vertical and horizontal tiles, as well as the tile size and
     * current tile index.
     */
    public void resetTileValues() {
      ...
      tileSize = new Dimension(
          (int) Math.ceil((float) getSourceRectangle().width / (float) hTiles), (int) Math
    ...
    /**
     * Updates the current tile on the Thumbnail. An area of the source Figure is painted
     * to an {@link Image}. That Image is then drawn on the Thumbnail. Scaling of the
     * source Image is done inside
     * {@link GC#drawImage(Image, int, int, int, int, int, int, int, int)} since the
     * source and target sizes are different. The current tile indexes are incremented and
     * if more updating is necesary, this {@link Runnable} is called again in a
     * {@link Display#timerExec(int, Runnable)}. If no more updating is required,
     * {@link #stop()} is called.
     */
    public void run() {
    ...
    /**
     * Starts this updater. This method initializes all the necessary resources and puts
     * this {@link Runnable} on the asynch queue. If this updater is not active or is
     * already running, this method just returns.
     */
    public void start() {
      if (!isActive() || isRunning())
        return;
    ...
    /**
     * Stops this updater. Also disposes of resources (except the thumbnail image which is
     * still needed for painting).
     */
    public void stop() {
      isRunning = false;
      if (thumbnailGC != null) {
  ...
  /**
   * Creates a new Thumbnail. The source Figure must be set separately if you use this
   * constructor.
   */
  public SafiThumbnail() {
    super();
  }
   ...
   *          The source figure
   */
  public SafiThumbnail(IFigure fig) {
    this();
    setSource(fig);
  }
   ...
   * in a way that maintains the source Figure's aspect ratio.
   * 
   * @param wHint
   *          The width hint
   * @param hHint
  ...
  /**
   * Returns the rectangular region relative to the source figure which will be the basis
   * of the thumbnail. The value may be returned by reference and should not be modified
   * by the caller.
   * 
   * @since 3.1
  ...
  /**
   * Sets the X and Y scales for the Thumbnail. These scales represent the ratio between
   * the source figure and the Thumbnail.
   * 
   * @param x
   *          The X scale
   * @param y
