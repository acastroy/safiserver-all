 ...
 */
public final class SafiScrollableThumbnail extends SafiThumbnail {

  private class ClickScrollerAndDragTransferrer extends MouseMotionListener.Stub implements
      MouseListener {
    ...
    public void mousePressed(MouseEvent me) {
      if (!(SafiScrollableThumbnail.this.getClientArea().contains(me.getLocation())))
        return;
      Dimension selectorCenter = selector.getBounds().getSize().scale(0.5f);
      Point scrollPoint = me.getLocation().getTranslated(getLocation().getNegated()).translate(
          selectorCenter.negate()).scale(1.0f / getViewportScaleX(), 1.0f / getViewportScaleY())
   ...
   */
  public SafiScrollableThumbnail() {
    super();
    initialize();
  }
   ...
   *          The Viewport
   */
  public SafiScrollableThumbnail(Viewport port) {
    super();
    setViewport(port);
    initialize();
  }
  ...
  private void reconfigureSelectorBounds() {
    ...
    Dimension vpSize = viewport.getClientArea().getSize();
    rect.setSize(vpSize);
