...
/**
 * New wizard selection tab that allows the user to select a registered 'New' wizard to be
 * launched.
 */
class SafiNewWizardNewPage implements ISelectionChangedListener {
  ...
  private SafiNewWizardSelectionPage page;
   ...
   * @param projectsOnly
   */
  public SafiNewWizardNewPage(SafiNewWizardSelectionPage mainPage,
      IWizardCategory wizardCategories, IWizardDescriptor[] primaryWizards, boolean projectsOnly) {
    this.page = mainPage;
    ...
    if (this.primaryWizards.length > 0) {
      if (allPrimary(wizardCategories)) {
        this.wizardCategories = null; // dont bother considering the categories as all
        // wizards are primary
        needShowAll = false;
      } else {
  ...
  protected FilteredTree createFilteredTree(Composite parent) {
    ...
    FilteredTree filterTree = new FilteredTree(composite, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL
        | SWT.BORDER, filteredTreeFilter);
    ...
    if (wizardCategories != null) {
      if (wizardCategories.getParent() == null) {
        ...
        /*
         * if(children.length>0){ inputArray.add(children[0]); }
         */
        // TODO:jong is here adding hack
        // this will hid all the other wizard except default
        for (IWizardCategory element : children) {

          if (element.getId().equals("org.eclipse.ui.Basic")) {
  ...
  /**
   * Expands the wizard categories in this page's category viewer that were expanded last
   * time this page was used. If a category that was previously expanded no longer exists
   * then it is ignored.
   */
   ...
   * <code>null</code> if the selectionEvent contains either 0 or 2+ selected objects.
   */
  ...
  /**
   * Store the current values of self's widgets so that they can be restored in the next
   * instance of self
   * 
   */
  ...
  /**
   * Selects the wizard category and wizard in this page that were selected last time this
   * page was used. If a category or wizard that was previously selected no longer exists
   * then it is ignored.
   */
  ...
  protected void storeExpandedCategories() {
    ...
    settings.put(STORE_EXPANDED_CATEGORIES_ID, (String[]) expandedElementPaths
        .toArray(new String[expandedElementPaths.size()]));
  }

  /**
   * Stores the currently-selected element in this page's dialog store, in order to
   * recreate this page's state in the next instance of this page.
   */
  protected void storeSelectedCategoryAndWizard() {
    Object selected = getSingleSelection((IStructuredSelection) filteredTree.getViewer()
        .getSelection());
