...
/**
 * New wizard selection tab that allows the user to either select a registered 'New'
 * wizard to be launched, or to select a solution or projects to be retrieved from an
 * available server. This page contains two visual tabs that allow the user to perform
 * these tasks.
 * 
 * Temporarily has two inner pages. The new format page is used if the system is currently
 * aware of activity categories.
 */
class SafiNewWizardSelectionPage extends WorkbenchWizardSelectionPage {
  private IWizardCategory wizardCategories;

  // widgets
  private SafiNewWizardNewPage newResourcePage;
   ...
   */
  public SafiNewWizardSelectionPage(IWorkbench workbench, IStructuredSelection selection,
      IWizardCategory root, IWizardDescriptor[] primary, boolean projectsOnly) {
  ...
  public void createControl(Composite parent) {
    ...
    newResourcePage = new SafiNewWizardNewPage(this, wizardCategories, primaryWizards, projectsOnly);
  ...
  /**
   * Since Finish was pressed, write widget values to the dialog store so that they will
   * persist into the next invocation of this wizard page
   */
  ...
  public boolean canFlipToNextPage() {
    // if the current page advertises that it does have pages then ask it via the super
    // call
    if (hasPages) {
  ...
  /**
   * Answers whether the currently selected page, if any, advertises that it may finish
   * early.
   * 
