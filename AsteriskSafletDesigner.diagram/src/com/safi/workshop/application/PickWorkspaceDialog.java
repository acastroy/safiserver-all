...
import za.co.quirk.layout.LatticeConstants;
import za.co.quirk.layout.LatticeData;
import za.co.quirk.layout.LatticeLayout;

/**
 * Dialog that lets/forces a user to enter/select a workspace that will be used when
 * saving all configuration files and settings. This dialog is shown at startup of the GUI
 * just after the splash screen has shown.
 * 
 * @author Emil Crumhorn
 */
public class PickWorkspaceDialog extends TitleAreaDialog {

  // the name of the file that tells us that the workspace directory belongs to our
  // application
  ...
  // this are our preferences we will be using as the IPreferenceStore is not available
  // yet
    ...
    clone.addListener(SWT.Selection, new Listener() {
      ...
      public void handleEvent(Event arg0) {
        try {
          ...
          // recursive check, if new directory is a subdirectory of our workspace, that's
          // a big no-no or we'll
  ...
  /**
   * This function will copy files or directories from one location to another. note that
   * the source and the destination must be mutually exclusive. This function can not be
   * used to copy a directory to a sub directory of itself. The function will also have
   * problems if the destination files already exist.
   * 
   * @param src
  ...
  public static void copyFiles(File src, File dest) throws IOException {
    ...
    if (src.isDirectory()) {
      ...
      try {
        ...
        IOException wrapper = new IOException("Unable to copy file: " + src.getAbsolutePath()
  ...
  /**
   * Ensures a workspace directory is OK in regards of reading/writing, etc. This method
   * will get called externally as well.
   * 
   * @param parentShell
   *          Shell parent shell
   ...
   * @param askCreate
   *          Whether to ask if to create the workspace or not in this location if it does
   *          not exist already
   * @param fromDialog
   *          Whether this method was called from our dialog or from somewhere else just
   *          to check a location
  ...
  public static String checkWorkspaceDirectory(Shell parentShell, String workspaceLocation,
      boolean askCreate, boolean fromDialog) {
    File f = new File(workspaceLocation);
    if (!f.exists()) {
      if (askCreate) {
        boolean create = MessageDialog.openConfirm(parentShell, "New Directory",
  ...
  /**
   * Checks to see if a workspace exists at a given directory string, and if not, creates
   * it. Also puts our identifying file inside that workspace.
   * 
   * @param wsRoot
