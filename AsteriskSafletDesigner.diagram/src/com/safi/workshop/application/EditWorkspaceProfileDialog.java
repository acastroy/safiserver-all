...
public class EditWorkspaceProfileDialog extends TitleAreaDialog {
  ...
  private final static String NAME_REGEX = "^[^\\\\\\./:\\*\\?\\\"<>\\|]{1}[^\\\\/:\\*\\?\\\"<>\\|]{0,254}$";
   ...
   * @param list
   */
  public EditWorkspaceProfileDialog(Shell parentShell,
      ChooseSafiServerWorkspaceData.SafiWorkspaceProfile profile, List<SafiWorkspaceProfile> list) {
    super(parentShell);
    ...
    if (StringUtils.isBlank(pathText.getText())) {
      ...
      if (profile != null && StringUtils.isNotBlank(profile.getName())) {
        ...
        if (!userDir.endsWith(File.separator))
          wsPath = userDir + File.separator + wsName;
        else
          wsPath = userDir + wsName;
  ...
  private boolean validate() {
    ...
    if (StringUtils.isBlank(nameText.getText())) {
      if (profile != null)
        setMessage("Name cannot be blank", IMessageProvider.ERROR);
      else
        setMessage("Please enter a Workspace name", IMessageProvider.INFORMATION);
      valid = false;
    } else if (!nameText.getText().trim().matches(NAME_REGEX)) {
      setMessage("Name cannot contain any of the following characters \\/:*?\"<>|",
    ...
    if (valid && StringUtils.isBlank(pathText.getText())) {
      if (profile != null)
        setMessage("Path cannot be blank", IMessageProvider.ERROR);
      else
        setMessage("Please enter a valid path", IMessageProvider.INFORMATION);
      valid = false;
    } else if (valid && !new File(pathText.getText().trim()).getName().matches(NAME_REGEX)) {
      setMessage("File names cannot contain any of the following characters \\/:*?\"<>|",
          IMessageProvider.ERROR);
      valid = false;
    } else if (valid) {
      ...
      if (valid && profile == null && f.exists() && !f.isDirectory()) {
        setMessage("File exists at " + f.getAbsolutePath() + " and is not a directory.",
            IMessageProvider.ERROR);
        valid = false;
    ...
    if (valid)
      setMessage("");
    if (okButton != null && okButton.getEnabled() != valid)
