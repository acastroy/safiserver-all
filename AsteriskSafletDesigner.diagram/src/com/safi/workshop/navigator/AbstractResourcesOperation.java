...
/**
 * An AbstractResourcesOperation represents an undoable operation that manipulates
 * resources. It provides implementations for resource rename, delete, creation, and
 * modification. It also assigns the workspace undo context as the undo context for
 * operations of this type. Clients may call the public API from a background thread.
 * 
...
abstract class AbstractResourcesOperation extends AbstractWorkspaceOperation {
  ...
  /*
   * The array of resource descriptions known by this operation to create or restore
   * overwritten resources.
   */
  protected ResourceDescription[] resourceDescriptions;

  /*
   * Return true if the specified subResource is a descendant of the specified super
   * resource. Used to remove descendants from the resource array when an operation is
   * requested on a parent and its descendant.
   */
  ...
  /**
   * Delete any resources known by this operation. Store enough information to undo and
   * redo the operation.
   * 
   * @param monitor
   ...
   * @param deleteContent
   *          <code>true</code> if the content of any known projects should be deleted
   *          along with the project. <code>false</code> if project content should not be
   *          deleted.
  ...
  /**
   * Recreate any resources known by this operation. Store enough information to undo and
   * redo the operation.
   * 
   * @param monitor
   *          the progress monitor to use for the operation
  ...
  /**
   * Compute the status for creating resources from the descriptions. A status severity of
   * <code>OK</code> indicates that the create is likely to be successful. A status
   * severity of <code>ERROR</code> indicates that the operation is no longer valid. Other
   * status severities are open to interpretation by the caller.
   * 
   * Note this method may be called on initial creation of a resource, or when a create or
   * delete operation is being undone or redone. Therefore, this method should check
   * conditions that can change over the life of the operation, such as the existence of
   * the information needed to carry out the operation. One-time static checks should
   * typically be done by the caller (such as the action that creates the operation) so
   * that the user is not continually prompted or warned about conditions that were
   * acceptable at the time of original execution.
   * 
   * @param allowOverwrite
   *          a boolean that specifies whether resource creation should be allowed to
  ...
  /**
   * Compute the status for deleting resources. A status severity of <code>OK</code>
   * indicates that the delete is likely to be successful. A status severity of
   * <code>ERROR</code> indicates that the operation is no longer valid. Other status
   * severities are open to interpretation by the caller.
   * 
   * Note this method may be called on initial deletion of a resource, or when a create or
   * delete operation is being undone or redone. Therefore, this method should check
   * conditions that can change over the life of the operation, such as the existence of
   * the resources to be deleted. One-time static checks should typically be done by the
   * caller (such as the action that creates the operation) so that the user is not
   * continually prompted or warned about conditions that were acceptable at the time of
   * original execution.
   */
  ...
  /**
   * Check the specified resources for read only state, and return a status indicating
   * whether the resources can be deleted.
   */
  IStatus checkReadOnlyResources(IResource[] resourcesToCheck) {
    // Check read only status if we are permitted
    // to consult the user.
    if (!quietCompute) {
      ReadOnlyStateChecker checker = new ReadOnlyStateChecker(getShell(null),
   ...
   * undoing or redoing this operation.
   * 
   * @param descriptions
   ...
   * 
   * @see
   * org.eclipse.ui.ide.undo.AbstractWorkspaceOperation#appendDescriptiveText(java.lang
   * .StringBuffer)
   */
  @Override
   ...
   * 
   * @return a scheduling rule appropriate for creating the resources specified in the
   ...
   * 
   * @return a scheduling rule appropriate for deleting the resources specified in the
   *         receiver.
   */
   ...
   * 
   * @see
   * org.eclipse.ui.ide.undo.AbstractWorkspaceOperation#setTargetResources(org.eclipse
