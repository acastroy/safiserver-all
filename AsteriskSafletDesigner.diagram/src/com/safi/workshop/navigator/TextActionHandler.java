...
/**
 * Handles the redirection of the global Cut, Copy, Paste, and Select All actions to
 * either the current inline text control or the part's supplied action handler.
 * <p>
...
public class TextActionHandler extends org.eclipse.ui.actions.TextActionHandler {
  ...
  private class DeleteActionHandler extends Action {
    ...
    public void runWithEvent(Event event) {
      if (activeTextControl != null && !activeTextControl.isDisposed()) {
        ...
        StringBuffer buf = new StringBuffer(text.substring(0, selection.x));
    ...
    public void updateEnabledState() {
      if (activeTextControl != null && !activeTextControl.isDisposed()) {
        setEnabled(activeTextControl.getSelectionCount() > 0
            || activeTextControl.getCaretPosition() < activeTextControl.getCharCount());
        return;
      }
      if (deleteAction != null) {
  ...
  /**
   * Creates a <code>Text</code> control action handler for the global Cut, Copy, Paste,
   * Delete, and Select All of the action bar.
   * 
   * @param theActionBars
   *          the action bar to register global action handlers for Cut, Copy, Paste,
  ...
  public void updateActionBars() {
    actionBars.setGlobalActionHandler(ActionFactory.CUT.getId(), textCutAction);
    actionBars.setGlobalActionHandler(ActionFactory.COPY.getId(), textCopyAction);
    actionBars.setGlobalActionHandler(ActionFactory.PASTE.getId(), textPasteAction);
    actionBars.setGlobalActionHandler(ActionFactory.SELECT_ALL.getId(), textSelectAllAction);
    actionBars.setGlobalActionHandler(ActionFactory.DELETE.getId(), textDeleteAction);
  }

  /**
   * Add a <code>Text</code> control to the handler so that the Cut, Copy, Paste, Delete,
   * and Select All actions are redirected to it when active.
   * 
   * @param textControl
  ...
  /**
   * Removes a <code>Text</code> control from the handler so that the Cut, Copy, Paste,
   * Delete, and Select All actions are no longer redirected to it when active.
   * 
   * @param textControl
   *          the inline <code>Text</code> control
   */
   ...
   * 
   * @param action
   *          the <code>IAction</code> to run for the Copy action, or <code>null</code> if
   *          not interested.
   */
  @Override
   ...
   * 
   * @param action
   *          the <code>IAction</code> to run for the Cut action, or <code>null</code> if
   *          not interested.
   */
  @Override
   ...
   * 
   * @param action
   *          the <code>IAction</code> to run for the Paste action, or <code>null</code>
   *          if not interested.
   */
  @Override
   ...
   * 
   * @param action
   *          the <code>IAction</code> to run for the Delete action, or <code>null</code>
   *          if not interested.
   */
  @Override
  ...
  /**
   * Update the enable state of the Cut, Copy, Paste, Delete, and Select All action
   * handlers
   */
