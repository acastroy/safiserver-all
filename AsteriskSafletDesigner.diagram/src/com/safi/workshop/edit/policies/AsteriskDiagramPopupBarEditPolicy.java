...
public class AsteriskDiagramPopupBarEditPolicy extends DiagramPopupBarEditPolicy implements
    PaletteListener {
  ...
  private class PopupBarLabelHandle extends Label implements Handle {
    ...
    /**
     * @see org.eclipse.draw2d.Figure#paintBorder(org.eclipse.draw2d.Graphics) paint a
     *      focus rectangle for the label if the mouse is inside the label
     */
    @Override
   ...
   * (non-Javadoc)
   * 
   * @seeorg.eclipse.gmf.runtime.diagram.ui.editpolicies.DiagramAssistantEditPolicy#
  ...
  /**
   * Listens to the owner figure being moved so the handles can be removed when this
   * occurs.
   * 
   * @author affrantz@us.ibm.com
   * 
  ...
  /**
   * Listens for mouse key presses so the popup bar can be dismissed if the context menu
   * is displayed
   * 
   * @author affrantz@us.ibm.com
   */
  ...
  static private int BALLOON_NUM_COLS = 20;

  /** Y postion offset from shape where the balloon top begin. */
  static private int ACTION_WIDTH_HGT = 40;
  ...
  static private int POPUPBAR_MOVE_FIGURE = 0x02; /*
                                                   * Ignore the first figureMoved event
                                                   * when creating elements inside a shape
                                                   * via a popup bar
                                                   */
  static private int POPUPBAR_DISPLAYATMOUSEHOVERLOCATION = 0x04; /*
                                                                   * Display the popup bar
                                                                   * at the mouse location
                                                                   * used by diagrams and
                                                                   * machine edit parts
                                                                   */
  static private int POPUPBAR_ONDIAGRAMACTIVATED = 0x10; /*
                                                          * For popup bars on diagram and
                                                          * machine edit parts, where we
                                                          * POPUPBAR_DISPLAYATMOUSEHOVERLOCATION
                                                          * , don't display popup bar
                                                          * until user clicks on surface
                                                          */
  static private int POPUPBAR_HOST_IS_CONNECTION = 0x20; /*
                                                          * For popup bars on connection
                                                          * edit parts
                                                          */
   ...
   * editpolicy (i.e. <code>fillPopupBarDescriptors</code> and by querying the modeling
   * assistant service for all types supported on the popup bar of this host. For those
   * types added by the modeling assistant service the icons are retrieved using the Icon
   * Service.
   */
  @Override
  protected void populatePopupBars() {
    ...
    for (Iterator iter = types.iterator(); iter.hasNext();) {
      ...
      if (type instanceof IElementType) {
        addPopupBarDescriptor((IElementType) type, IconService.getInstance().getIcon(
            (IElementType) type));
      }
    }
  }
   ...
   * descrioptors if they have customized tools that cannot be done using the type along
   * with the modeling assistant service.
   */
  @Override
  ...
  /**
   * Adds popup bar descriptors for all the shape tools in the palette container of the
   * last active palette tool. Subclasses may override if they wish to customize this
  ...
  protected void fillBasedOnLastActivePaletteTool() {
    if (theLastTool == null)
      return;
    // if (theLastTool.getId() == Note
    if ("noteTool".equals(theLastTool.getId()) || "zoomTool".equals(theLastTool.getId()))
      return;
    ...
    PaletteRoot realPaletteRoot = getHost().getViewer().getEditDomain().getPaletteViewer()
        .getPaletteRoot();
   ...
   * Subclasses may override if they wish to customize this behavior for their diagram.
   * 
   * @param palContainer
  ...
  private Image convert(Image srcImage) {
    ...
    Image convertedImage = ImageDescriptor.createFromImageData(srcImageData).createImage(
        srcImage.getDevice());
  ...
  /**
   * default method for plugins which passes along the PopupBarTool as the tool to be
   * used.
   * 
   * @param elementType
   * @param theImage
  ...
  private void initPopupBars() {
    ...
    // myBalloon.setLayoutManager(new GridLayout(15, true));
    // myBalloon.setPreferredSize(new Dimension(400, 600));
    // int iTotal = ACTION_WIDTH_HGT * theList.size() + ACTION_MARGIN_RIGHT;
    int iTotal = ACTION_WIDTH_HGT * BALLOON_NUM_COLS + ACTION_MARGIN_RIGHT;

    int rowHeight = ACTION_WIDTH_HGT + ACTION_BUTTON_START_Y;

    int totalHeight = ((int) Math.ceil(theList.size() / (double) BALLOON_NUM_COLS)) * rowHeight + 2
        * ACTION_BUTTON_START_Y;
    getBalloon().setSize(iTotal, totalHeight);
    ...
    for (Iterator iter = theList.iterator(); iter.hasNext();) {
      ...
      if (currentCol++ >= BALLOON_NUM_COLS) {
   ...
   * we want to display the popup bar at the mouse location.
   * 
   * @param referencePoint
   *          The reference point which may be used to determine where the diagram
   *          assistant should be located. This is most likely the current mouse location.
   * @return Point
   */
  private Point getBalloonPosition(Point referencePoint) {
    ...
    if (atMouse) {
      ...
      thePoint.y -= (getBalloon().getSize().height - 2 * ACTION_BUTTON_START_Y);// + 2 *
      // ACTION_BUTTON_START_Y);
      ...
      if (ctrl1 instanceof FigureCanvas) {
        ...
        clippedX = willBalloonBeClippedX(thePoint, vpRect, dim);
        clippedY = willBalloonBeClippedY(thePoint, vpRect, dim);
      }
      ...
      if (clippedY || clippedX) {
        ...
        Dimension dim = getBalloon().getSize();
        if (clippedX) {
          int offsetX = dim.width + ACTION_BUTTON_START_X;
          thePoint.x = rcBounds.right() - offsetX;
        }
        if (clippedY) {
          // int offsetY = dim.height + ACTION_WIDTH_HGT;
          // thePoint.y = offsetY;
          thePoint.y = rcBounds.y;
        }
      }
  ...
  private boolean willBalloonBeClipped(Point pnt) {
    ...
    if (ctrl1 instanceof FigureCanvas) {
      ...
      Rectangle vpRect = vp.getClientArea();
      Dimension dim = getBalloon().getSize();
      if ((pnt.x + dim.width) >= (vpRect.x + vpRect.width) || (pnt.y - dim.height <= vpRect.y)) {
        return true;
      }
    }
    return false;
  ...
  private boolean willBalloonBeClippedX(Point pnt, Rectangle vpRect, Dimension dim) {
  ...
  private boolean willBalloonBeClippedY(Point pnt, Rectangle vpRect, Dimension dim) {
    if (pnt.y <= vpRect.y) {
      return true;
    }
    return false;
  ...
  /**
   * Gets the amount of time to wait before showing the popup bar if the popup bar is to
   * be shown at the mouse location {@link #getIsDisplayAtMouseHoverLocation()}.
   * 
