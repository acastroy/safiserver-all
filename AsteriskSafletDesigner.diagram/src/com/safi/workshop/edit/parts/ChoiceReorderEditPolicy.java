...
public class ChoiceReorderEditPolicy extends FlowLayoutEditPolicy {

  public ChoiceReorderEditPolicy() {
  ...
  protected Command createMoveChildCommand(final EditPart child, final EditPart after) {
    ...
    if (child instanceof CaseItemEditPart && child != after
        && (after == null || after instanceof CaseItemEditPart)) {

      final Choice choice = (Choice) getHostObject();
      AbstractTransactionalCommand reorderCommand = new AbstractTransactionalCommand(
          ((IGraphicalEditPart) getHost()).getEditingDomain(), "reorder choice kids", null) {

        @Override
        protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
            throws ExecutionException {
          CaseItem childItem = (CaseItem) ((ActionstepSubItem) child).getItemModel();
          EList<CaseItem> list = choice.getChoices();
          ...
          if (displacement != 0) {
            ...
            // compartment.refresh();
            ((ActionstepSubitemContainer) getHost()).updateSizeIfNecessary();
            compartment.refresh();
