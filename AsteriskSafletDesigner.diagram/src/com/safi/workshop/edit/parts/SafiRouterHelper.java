...
/**
 * Manager class which determines which shortest path connection router to use for a given
 * <code>Connection</code>.
 * 
 * @author sshaw
 * 
 */
public class SafiRouterHelper {

  static private SafiRouterHelper sprm = new SafiRouterHelper(false);

  /**
   * @return the <code>SafiRouterHelper</code> singleton instance
   */
  static public SafiRouterHelper getInstance() {
    return sprm;
  }
  ...
  private SafiRouterHelper(boolean useGEFRouter) {
    super();
   ...
   *          connection or not.
   * @return <code>true</code> is it is a feedback connection, <code>false</code>
   *         otherwise.
   */
   ...
   * @return the <code>PointList</code> that is the list of points that are a direct
   *         mapping of the constraint points.
   */
   ...
   * @param conn
   *          the <code>Connection</code> that is to be routed.
   * @return the <code>PointList</code> that is the list of points that represent the
   *         closest distance possible to route the line.
   */
   ...
   * @param conn
   *          the <code>Connection</code> that is to be routed.
   * @return the <code>PointList</code> that is the list of points that are avoiding all
   *         the possible obstructions in the container for the connection.
   */
  public PointList routeAroundObstructions(Connection conn) {
    ...
    if (useGEFRouter) {
      ...
      ShortestPathConnectionRouter spcr = SafiRouterHelper.getInstance().getRouter(conn);
      if (spcr == null)
      ...
      // newLine.setPoint(ptRef.getCopy().getTranslated(10, 0), 1);
      ptRef = conn.getTargetAnchor().getReferencePoint();
      conn.translateToRelative(ptRef);
      newLine.setPoint(ptRef, newLine.size() - 1);
      // newLine.setPoint(ptRef.getCopy().getTranslated(-10, 0), newLine.size() - 2);
      ...
      if (collectObstructs.size() > 0) {
        ...
        while (bRouted && !collapsedRects.isEmpty()) {
          ...
          while (listIter.hasNext()) {
            ...
            PointList routedPoly = PointListUtilities.routeAroundRect(newLine, rObstruct, 0, false,
                inflate);
  ...
  public void prettify(PointList newLine) {
    if (newLine.size() < 2)
      return;
    Point first = newLine.getFirstPoint();
    Point next = newLine.getPoint(1);
    if (first.x == next.x) {
      Point np1 = first.getCopy().translate(20, 0);
      Point np2 = next.getCopy().translate(20, 0);
      newLine.insertPoint(np1, 1);
      newLine.insertPoint(np2, 2);
    }
  }

  /**
   * Finds all the children shapes of the parent figure passed in that are in the way of
   * the connection. This method will dig into children of container shapes if one of the
   * connection ends is also in that container.
   * 
   * @param connection
   *          the connection being routed
   * @param connectionRect
   *          the rectangle representing the connection bounds that is used to determine
   *          if a shape intersects with the connection
   * @param obstructionsToReturn
   *          the list of figures that the connection should be routed around
   */
  protected void collectObstructions(Connection connection, Rectangle connectionRect,
      List obstructionsToReturn) {
    ...
    for (Iterator iter = containerFiguresToSearch.iterator(); iter.hasNext();) {
      IFigure containerFigure = (IFigure) iter.next();

      for (Iterator iterator = containerFigure.getChildren().iterator(); iterator.hasNext();) {
        IFigure childFigure = (IFigure) iterator.next();

        if (true || !figuresToExclude.contains(childFigure)) {
  ...
  /**
   * @param conn
   *          the <code>Connection</code> that is to have used to determine the end points
   *          for reseting the <code>newLine</code> parameter.
   * @param newLine
   *          the <code>PointList</code> to reset the end points of to be on the edge of
   *          the connection source and target nodes.
   */
  ...
  /**
   * This method will collapse all the rectangles together that intersect in the given
   * List. It utilizes a recursive implementation.
   */
  ...
  private IFigure getRouterContainerFigure(Connection conn) {
    ...
    if (sourcefigContainer == targetfigContainer) {
      ...
      int targetDistance = (int) PointListUtilities.getPointsLength(conn.getPoints())
          - sourceDistance;
  ...
  /**
   * findContainerFigure Recursive method to find the figure that owns the children the
  ...
  private int findDistanceToEndRect(PointList points, Rectangle endRect) {
    ...
    boolean foundSourceDistance = PointListUtilities.findIntersections(points, PointListUtilities
        .createPointsFromRect(endRect), intersections, distances);
