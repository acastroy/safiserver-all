...
public abstract class ToolstepEditPart extends AbstractBorderedShapeEditPart {
  ...
  protected static final Status OK_STATUS = new Status(IStatus.OK, AsteriskDiagramEditorPlugin.ID,
      null);
  ...
  public ToolstepEditPart(View view) {
    super(view);
    // ActionStep tool = getActionStep();
    // EStructuralFeature feature = tool.eClass().getEStructuralFeature("active");
    // feature.eAdapters().add(this);
  ...
  public void addNotify() {
    // TODO Auto-generated method stub
    super.addNotify();
    setMinimized(AsteriskDiagramEditorPlugin.getInstance().getPreferenceStore().getBoolean(
        PreferenceConstants.PREF_ACTIONSTEPS_MINIMIZED));
  ...
  protected void createDefaultEditPolicies() {
    ...
    // for view opening properties or dialog open call
  ...
  protected void handleNotificationEvent(Notification notification) {
    ...
    if (obj instanceof EStructuralFeature) {
      ...
      if ("active".equals(feature.getName())) {
      ...
      } else if (!((HandlerEditPart) getParent()).isDebug()
        ...
        // if (notification.getNewValue() instanceof DynamicValue){
        // int selected = getSelected();
        // handleMajorSemanticChange();
        // if (selected == EditPart.SELECTED_PRIMARY){
  ...
  protected void showActiveFeedback() {
    AsteriskDiagramEditor editor = (AsteriskDiagramEditor) ((DiagramEditDomain) getEditDomain())
        .getEditorPart();
    // ActionFactory.ACTIVATE_EDITOR.create();
    editor.getSite().getPage().activate(editor);
    getViewer().reveal(this);
    // AsteriskDiagramEditorPlugin.getInstance().getWorkbench().getActiveWorkbenchWindow().getActivePage().activate(editor);
  ...
  protected IFigure createActiveFeedbackFigure() {
    ...
    // r.setForegroundColor(ColorConstants.yellow);
    // Rectangle bounds = getFigure().getBounds().getCopy();
    // getFigure().translateToAbsolute(bounds);
    // r.translateToRelative(bounds);
    // r.setBounds(bounds);
    return r;
  }
        ...
        try {
          ...
          if (StringUtils.isBlank(varNameStr)) {
            return new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID,
                "Couldn't resolve variable name");
          }
          ...
          if (v == null)
            return new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID,
                "Couldn't resolve variable named " + varNameStr);
        } catch (ActionStepException e) {
          e.printStackTrace();
          return new Status(IStatus.ERROR, AsteriskDiagramEditorPlugin.ID,
              "Couldn't find variable: " + e.getLocalizedMessage());
        }
      }
    }
  ...
  public BorderItemLocator configureLocator(IFigure fig) {
    BorderItemLocator locator = new com.safi.asterisk.figures.OutputFigure.TerminalLocator(
  ...
  public ActionStep getActionStep() {
    ...
    // Object mod = getModel();
    // if (mod instanceof ActionStep) return (ActionStep) mod;
    // if (mod instanceof View) {
    // return (ActionStep) ((View) mod).getElement();
    // }
    // return null;
  }
  ...
  protected final NodeFigure createNodePlate() {
    DefaultSizeNodeFigure result = new DefaultSizeNodeFigure(
        getMapMode().DPtoLP(getDefaultWidth()), getMapMode().DPtoLP(getDefaultHeight())) {
      @Override
      ...
      public Dimension getDefaultSize() {
        if (minimized)
          return ToolstepEditPart.this.getMinimumSize();
