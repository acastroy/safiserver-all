...
public class SafiPromptForConnectionAndEndCommand extends PromptForConnectionAndEndCommand {
  ...
  public SafiPromptForConnectionAndEndCommand(CreateConnectionRequest request,
      HandlerEditPart containerEP) {
  ...
  protected PopupMenu createPopupMenu() {
    ...
    if (connectionMenuContent == null || connectionMenuContent.isEmpty()) {
      ...
      if (handlerEP != null && handlerEP.get() != null) {
        ...
        // PaletteRoot root =
        // ((AsteriskDiagramEditor)((DiagramEditDomain)((DiagramGraphicalViewer)
        // ((HandlerEditPart)
        // handlerEP).getViewer()).getDiagramEditDomain()).getEditorPart()).getCurrentPaletteRoot();
      ...
      cachedMenu = new WeakReference<PopupMenu>(new PopupMenu(menuContent, labelProvider) {
        ...
        public Object getResult() {
          ...
          if (endResult == null) {
            ...
            resultList.add((endResult instanceof List) ? ((List) endResult).get(((List) endResult)
                .size() - 1) : endResult);
            return resultList;
          }
        }
      ...
      if (!menuContent.isEmpty()) {
        cachedMenu = new WeakReference<PopupMenu>(new PopupMenu(menuContent,
  ...
  private void buildPaletteItemMap(PaletteContainer parent, Map<MetamodelType, String> paletteMap) {
    ...
    for (Object child : parent.getChildren()) {
      if (child instanceof AsteriskPaletteFactory.NodeToolEntry) {
        ...
        buildPaletteItemMap((PaletteContainer) child, paletteMap);
    }
  }
  ...
  protected ILabelProvider getConnectionAndEndLabelProvider(Object connectionItem) {
    if (connectionAndEndLabelProvider == null)
      connectionAndEndLabelProvider = new SafiConnectionAndEndLabelProvider(connectionItem);
  ...
  protected ILabelProvider getEndLabelProvider() {
    if (endLabelProvider == null)
      endLabelProvider = new SafiEndLabelProvider();
  ...
  protected ILabelProvider getConnectionLabelProvider() {
    if (connectionLabelProvider == null)
      connectionLabelProvider = new SafiConnectionLabelProvider();
