...
/**
 * A field editor for a combo box that allows the drop-down selection of one of a list of
 * items.
 * 
...
public class ComboFieldEditor extends FieldEditor {
   ...
   * be arranged as: { {name1, value1}, {name2, value2}, ...}
   */
  ...
  protected String getValueForName(String name) {
    for (String[] entry : this.fEntryNamesAndValues) {
  ...
  protected void updateComboForValue(String value) {
    this.fValue = value;
    for (String[] entryNamesAndValue : this.fEntryNamesAndValues) {
      if (value.equals(entryNamesAndValue[1])) {
        this.fCombo.setText(entryNamesAndValue[0]);
        return;
      }
    }
