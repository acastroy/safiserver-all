...
/**
 * A special abstract preference page to host field editors and eny other controls with
 * flexible layout.
 * <p>
 ...
 * @author scheglov_ke
 */
public abstract class FieldLayoutPreferencePage extends PreferencePage implements
  ...
  /**
   * Recomputes the page's error state by calling <code>isValid</code> for every field
   * editor.
   */
   ...
   * <p>
   * This framework method must be implemented by concrete subclasses. Any subclass
   * returning a <code>Composite</code> object whose <code>Layout</code> has default
   * margins (for example, a <code>GridLayout</code>) are expected to set the margins of
   * this <code>Layout</code> to 0 pixels.
   * </p>
   * 
   * @param parent
  ...
  /**
   * The field editor preference page implementation of an <code>IDialogPage</code> method
   * disposes of this page's controls and images. Subclasses may override to release their
   * own allocated SWT resources, but must call <code>super.dispose</code>.
   */
  @Override
  public void dispose() {
    super.dispose();
    if (this.fields != null) {
      ...
      while (e.hasNext()) {
        ...
        pe.setPage(null);
        /*
         * $codepro.preprocessor.elseif version < 3.0 $ pe.setPreferencePage(null);
  ...
  /**
   * The field editor preference page implementation of this <code>PreferencePage</code>
   * method saves all field editors by calling <code>FieldEditor.store</code>. Note that
   * this method does not save the preference store itself; it just stores the values back
